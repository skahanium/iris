...
- `WebPreferred`：普通外部事实、推荐、比较和一般研究保留模型自主调用；日常时效的 `VolatileExternalFact` 必须先取得真实检索观察。缺少正文可形成准确的限制答复，不能冒充已核实当前事实。
- `WebRequired`：用户明确要求联网或核实、指定 URL，以及医疗、法律、党纪、签证等高利害当前事实。缺少当前 Run 证据时不得伪造适用结论。

联网开关只决定授权。现有 ToolSurfacePlan 从冻结原因、验证要求与实际授权派生最低观察义务；该义务与 VerificationRequirement 的证据要求分开，不能互相增权。提示词、Host 起步与终态检查共同消费该策略。
...
Host 与模型看到的是同一循环状态的两个投影，不是两份预算。Host 持有精确计数、超时阶段与 Provider 尝试；模型只取得当前能否继续、是否必须综合、失败类型和可采取的下一步。终态身份同样是显式类型而非正文推断，校验、引用绑定与提交消费同一类型，详见 [04](./04-adaptive-agent-loop-and-tool-contracts.md) 第 5.1 节。
