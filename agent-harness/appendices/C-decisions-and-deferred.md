...
| DEC-029 | 授权、最低观察与证据要求独立派生       | 2026-09-08 生产复现证明：放宽日常题无摘录降级，不能同时取消实际检索；提示词、执行和终态共享 ToolSurfacePlan 策略 |
| DEC-030 | 调用修复、服务失败与研究无进展分别计数 | 未执行提议不能冒充搜索失败；具体拒绝原因及累计退出摘要必须可诊断，恢复仍受同一预算约束                           |

日常时效的地域默认值是明确产品偏好，不是用户定位：本轮明确地区 > 当前话题中用户确认的地区 > 中国大陆；地域无关事实不套用默认范围。电影和音乐的大陆可用范围不等于仅国产作品。Host 只对可确定无额外限定的首轮泛问补默认地区，保留其他原始约束，由同一模型循环处理复杂地区和追问；不得把历史助手回答、笔记或附件自动拼进查询。来源与目标范围不符时必须继续调整查询，不能把初步观察视为范围已满足。

DEC-025 补充（2026-09-08）：真实工具执行会禁止跨 Provider 续接，但不应禁止尚无可见正文时针对瞬态错误的一次同模型重试；重试复用原模型请求及工具观察，不重放工具。400/422 等请求拒绝不按瞬态失败盲目重试，也不能仅凭 HTTP 状态归责于 Harness 或厂商。错误分类、可用 HTTP 状态和恢复决策写入既有安全诊断，界面只陈述已确认的阶段；官方示例与生产请求对照的结果单独记录，未复现的适配器假设不进入生产补丁。

DEC-025 补充（2026-09-09）：工具名称、标识或 JSON 提议未实际派发时，只提供同模型的有界修复；修复耗尽后关闭业务工具并综合已有观察，不触发备用模型。备用模型只处理确认的瞬态模型服务失败，且只可在无副作用、无后续工具的条件下综合已取得资料。

DEC-030（2026-09-09）：已信任、启用且配置未漂移的外部只读 binding 是普通任务能力，接收 Run 时自动冻结；它不改变 Web 授权或强制外部来源要求。公开 HTTPS URL 可直接进入统一安全读取边界，只有成功正文才成为证据。
