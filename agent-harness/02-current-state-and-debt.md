...
另有必须先明确持久化整改边界的旧缺陷：`app.rs` 的 CAS `OnceLock` 在换 Vault 后仍返回首次初始化的 store；`files.path` 是全局身份，版本通过 `versions.file_id ON DELETE CASCADE` 关联它；`indexer/scan.rs` 的 prune 和 `commands/file.rs` 的目录移动会删除缺失旧路径的身份行。仅抽取工具适配器不能修复跨库同名路径与历史归属。未确认历史归属的记录不得按当前 Vault 猜测迁移或删除。真实差异确认、执行回执持久化、编辑器实际接纳链路及独立 Agent 撤销仍待后续完成。
