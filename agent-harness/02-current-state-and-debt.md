...
复审基线为 `99187cf1`，覆盖 `76afc57e..99187cf1`。本节替代此前“①–⑧、⑩ 已修并各自带反向验证”的结论；局部测试通过不能覆盖生产路径反例。方案 A 与 B 已提交（`740f6d61`、`248b1e19`），C 已按同一方案落地，D 仍待实施，任务分解和验收条件见[针对性修复方案](../docs/superpowers/plans/2026-09-14-agent-correctness-remediation.md)。

| 原事项             | 当前状态           | 复审核实的事实                                                                                                                                                                                                                  |
| ------------------ | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ① 终态限制说明识别 | 已实现（已提交）   | `AgentToolLoopOutcome` 携带显式 `AgentTerminalType`（模型回答 / 修复后模型回答 / Host 证据受限），校验、引用绑定与提交消费同一类型；新 Run 不再按正文前缀授予豁免，旧正文前缀识别隔离在 `run_engine/legacy_terminal_records.rs` |
| ② JSON 载荷收缩    | 已实现（已提交）   | 以 JSON 值收缩正文或完整列表尾项；最小信封超限显式收束，不再切割序列化文本                                                                                                                                                      |
| ③ 中文查询         | 已实现（已提交）   | 单个 CJK 查询可命中；词组展开被括号限制在自身，多词组以显式 `AND` 连接，不再丢 `AND` 约束                                                                                                                                       |
| ④ 相关性与选段     | 已实现（已提交）   | 词项覆盖选段保留；BM25 分数转换改为 `max(-rank,0)/(1+max(-rank,0))`，层内顺序与 SQL 顺序一致；确定性 RAG 融合门保持通过                                                                                                         |
| ⑤ 阅读进展         | 已实现（已提交）   | 资源、快照 hash、实际范围和单位共同构成阅读身份；空窗口和重放不增加进展                                                                                                                                                         |
| ⑥ 检索状态         | 部分实现           | 已有有界 `retrievalStatus`；模型观察侧的内部计量已按 B 移除，真实回答质量仍未校准                                                                                                                                               |
| ⑦ 范围下推         | 已实现（已提交）   | 路径前缀改为 `substr(path,1,length(?)) = ? COLLATE BINARY`，`_`、`%` 与大小写按字面处理；FTS 与 metadata 复用同一谓词，事后过滤保留                                                                                             |
| ⑧ 网页续读         | 已实现（已提交）   | MCP/native 使用同 Run 有界快照；最终窗口先适配再登记，指针与 ID 引用绑定可见摘录                                                                                                                                                |
| ⑨ Host 起步        | 已实现，质量未校准 | 自然时效与严格检索均可能先取 Host 最低观察；历史依赖问题可能浪费预算和被早期候选影响，设计取舍仍须接受质量检验                                                                                                                  |
| ⑩ 工具观察压缩     | 已实现（已提交）   | 保留原状态、最近整批及回执；已压缩观察经权限重检可恢复，逻辑计费且不重执行                                                                                                                                                      |

FTS 三项（查询分组、分数方向、范围谓词）的具体合同见 [04 的 5.2 节](./04-adaptive-agent-loop-and-tool-contracts.md)，命名回归与生产入口见[附录 A](./appendices/A-status-and-test-traceability.md)。
...
- 精确预算长期作为 `loopObservation` 进入模型；正文中的“web_fetch 还可三次”把 network 分类额度误解释成单工具额度。超时和预算注入早于本轮重构；没有证据表明政治话题触发特殊限制。B 已把模型观察改为「能否继续 / 是否必须综合 / 失败类型 / 下一步」，Host 继续持有精确计数，但真实回答是否还会复述内部额度仍须实测。
