...
> 本轮施工事实补充至 2026-09-07 工作树；未提交代码不得被当作已部署能力。
...
移动与永久清理现已分别使用职责受限的私有恢复检查点，不与 Harness 合并成新存储框架。移动的独立字节恢复材料、目标物理身份复验与已应用回执已有定向回归；独立复审仍阻止交付：符号链接别名可能使实际路径与版本/索引身份分裂，须在移动副作用前统一拒绝。清理在暂存、数据库提交与重启收尾之间仍需复验精确历史归属，不能把归属冲突吞成暂时元数据失败。对应修复正在进行。真实差异确认、执行回执持久化、编辑器实际接纳链路及独立 Agent 撤销仍待后续完成，不得以局部通过掩盖。
...
编辑器异步图片已绑定原文档基线与选择位置，避免下载完成后插入另一文档或重放原粘贴事件；保存队列保留“旧写入实际成功、新编辑仍 dirty”的真实磁盘基线，迟到失败不能污染同路径重开的记录，锁定/Vault/基线冲突也不再自动重试。`file_write` 新增可选的发起 Vault 与已读 hash 前置条件，在同一保存锁内校验；空 hash 明确表示目标必须不存在，普通保存不因此生成手动版本。普通和涉密路径共用解码后的 hash 语义，涉密落盘仍加密。这一后端接口尚未由现代编辑器完整传入，不能据此宣称保存屏障、跨文档会话隔离或 Agent 回执接纳已经完成。

会话清理增加了事务与非终态拒绝回归，但完整生命周期复审发现两项缺口仍待关闭：`cancelled` 可能先于后台执行真正退出，不能仅凭终态删除 Run 及其回执；撤回清除摘要后，现有压缩入口在缺少摘要时直接跳过，保留历史超出近期窗口时会失去覆盖。会话撤回不能回滚 Markdown，删除会话也不能清除版本和回收站恢复材料。
