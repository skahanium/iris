...
复审基线为 `99187cf1`，覆盖 `76afc57e..99187cf1`。本节替代此前“①–⑧、⑩ 已修并各自带反向验证”的结论；局部测试通过不能覆盖生产路径反例。方案 A 与 B 已进入工作树，C/D 仍待实施，任务分解和验收条件见[针对性修复方案](../docs/superpowers/plans/2026-09-14-agent-correctness-remediation.md)。

| 原事项             | 当前状态             | 复审核实的事实                                                                                                                                                                                                                  |
| ------------------ | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ① 终态限制说明识别 | 已实现（工作树）     | `AgentToolLoopOutcome` 携带显式 `AgentTerminalType`（模型回答 / 修复后模型回答 / Host 证据受限），校验、引用绑定与提交消费同一类型；新 Run 不再按正文前缀授予豁免，旧正文前缀识别隔离在 `run_engine/legacy_terminal_records.rs` |
