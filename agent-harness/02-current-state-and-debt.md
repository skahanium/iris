...
- `agent:eval:contract` 只证明确定性合同；`agent:eval` 还要求两条各 6 Run 的真实路由结果和逐场景人工评分，合计不超过 12 Run，全部通过后才原子写产品报告。真实执行失败、预算违规或质量未通过的报告也会先持久化并签名，再由严格产品验证器拒绝，避免“失败越严重越没有证据”。
