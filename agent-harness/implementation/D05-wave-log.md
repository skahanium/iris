...
- **六项依据**：1＝2A 波 unknown 差异交付与确认卡禁批门闩（`71731801`）；2＝同波确认卡门闩（差异可见前禁批、拒绝可点）；3＝`Q14` 关条（`10d5f378`，两向证据分别验证）；4＝`C24`／`C25` 合同维持（第三、四、五波）；5＝纯编辑零检索机械钉（`9ca06b8c`）＋十题 T01–T04；6＝`N23` 证据波（`5272f640`）。
- **诚实注记**：第 1 项当时的落地只交付了**差异**，「与不确定项」一半经 2026-09-24 外部评审（F3）指出缺失，由治理收口波补齐交付；本条不改写关包事实，只登记补课去向。
- **关包动作**：清单全绿后另批关包——registry `work=done`／`implementation=present`／`verification=passed`（`72ca0dba`）。`passed` 只表示本包清单已绑定，不代标 `N04`／`C24`／`C25`／`K15` 的 passed，不表示 `V05` 全矩阵或 `V06` 语义验收。合同正文的 2026-09-23 批口径同步标注为历史（见本条对应提交）。
- **仍不关**：`D04`／`Q05`／`Q17`／`G02`／`G03`；不代标任何组件 passed。
