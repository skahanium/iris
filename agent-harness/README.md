...
- `X01`／`X02` 与 `D01`–`D03` 已按各自机械证据范围登记 `verification.state=passed`；C26／C27／E01–E03／V07 的通过与否以 `registry.json.verify` 为准，不把机械记录标成语义质量通过；
- 工作包 `work.state` 以 `catalog.mjs` 为准：`D01`–`D03` 为 `done`，`D04`、`D05` 为 `active`，`D06` 为 `planned`。
