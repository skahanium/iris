...
日常 `VolatileExternalFact` 走 `WebPreferred`，但独立要求实际搜索与有界候选正文读取；正常限制答复不依赖摘录，检索义务不得被“可无摘录完成”取消。`HighStakesCurrentFact`、CitationCheck 或用户明确要求交叉核实才要求官方来源或两个独立域名。现代消息的显式空 evidence 选择不得在历史重载时恢复为整 Run 来源。

未派发提议、服务失败和真实执行无进展分别计数。提议拒绝必须反馈具体原因与可用参数合同，最多两轮连续修复；不消耗工具额度、不计为已检索。所有恢复共享现有模型、工具和 network 预算并预留最终综合。最终生命周期完成、能力受限和回答质量必须分开验收。

## 6. 澄清与确认

- 问题范围、偏好、地点、语言、目标笔记等普通信息通过自然 assistant 消息澄清。
- 如果模型可以在回答中披露合理默认范围，就不强制追问。地域敏感的日常时效问题默认中国大陆，本轮明确范围及当前话题用户已确认范围优先；城市级场次、附近服务等仍需实际城市。
