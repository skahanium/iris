...
### 5.2 本地 FTS 检索语义（2026-09-14 收敛）

- 查询编译：每个用户词组先独立编译，再以显式 `AND` 连接；每个词组自身用括号包围。CJK 词组展开为「bigram 短语 OR 原样词」两种候选，被括号限制在该词组内。FTS5 的裸 `OR` 会跨界结合，`劳动合同 风险` 曾因此被解析成 `劳动… OR (劳动合同 AND 风险)`，丢失 `AND` 约束。清理规则与索引兼容不变，不引入查询 DSL，也不要求重建索引。
- 分数方向：FTS5 `bm25()` 越负越好。层内分数用 `strength = max(-rank, 0)`、`score = strength / (1 + strength)`，单调、有界，只表示层内相关性，不是概率。融合仍由既有加权 RRF 与 MMR 决定，权重、来源去重与 top-10 单源上限不变；但 `weighted_rrf` 会先按该分数给层内候选排名，反转的投影会重排融合本身。
- 范围谓词：路径前缀用参数化二进制字符串比较 `substr(path, 1, length(?)) = ? COLLATE BINARY`，与 `RetrievalScope::matches_path` 的字面前缀语义一致；精确路径继续用参数化 `IN`。`LIKE 'prefix%'` 会把前缀里的 `_`、`%` 当通配符并折叠 ASCII 大小写，选出范围本身拒绝的路径，并用小 `LIMIT` 的候选池换取这些行。FTS 与 metadata 两层复用同一谓词；最终 `filter_packets_by_scope` 仍保留，下推只修召回，不改授权。
- metadata 层自身没有相关性信号，候选按 `path` 排序后再 `LIMIT`；无排序时 `LIMIT` 返回的是扫描顺序，同一候选池可能每次都取到同一批无关行。
- 不增加「结果不足则全库重搜」的补救分支，也不改 SQLite 连接 PRAGMA。
