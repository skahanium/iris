...
describe("IPC boundary", () => {
  ...
  it("does not expose retired maintenance commands", () => {
    const ipc = read("src/lib/ipc.ts");
    expect(ipc).toContain("export async function settingsReset");
    expect(ipc).toContain('invoke("settings_reset"');
    expect(ipc).not.toContain("version_cleanup_cmd");
    expect(ipc).not.toContain("llm_providers");
    expect(existsSync("src-tauri/src/commands/llm.rs")).toBe(false);
  });
