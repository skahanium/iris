...
describe("useEditorStats session char delta", () => {
  it("keeps per-session accum when switching active session", () => {
    ...
    act(() => {
      ...
      result.current.applySessionCharDelta("session-a", { added: 5, removed: 0 });
    });

    act(() => {
      result.current.setActiveEditorSession("session-b");
      result.current.resetSessionCharDelta("session-b", 200);
      result.current.applySessionCharDelta("session-b", { added: 2, removed: 1 });
    });
  ...
  it("hides session delta when character count matches open baseline", () => {
    ...
    act(() => {
      ...
      result.current.applySessionCharDelta("session-a", { added: 1, removed: 0 });
      result.current.applySessionCharDelta("session-a", { added: 0, removed: 1 });
      result.current.updateEditorStats({ characterCount: 10, readingMinutes: 1 });
    });
