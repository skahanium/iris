...
- **职责**: 所有需要系统权限或原生性能的操作
- **关键 crate**:
  - `rusqlite` — SQLite 数据库操作（向量检索优先 sqlite-vec vec0，不可用时 cosine fallback）
...
- **SQLite 数据库**: 索引和缓存层。所有数据均可从 `.md` 文件重建。
...
用户输入自然语言查询
  ...
  → Rust Search Engine:
      ...
      2. sqlite-vec 虚拟表 vector Top-K（优先）
      3. cosine fallback（vec 不可用时）
      4. 返回 Top-K 结果 (文件路径 + 片段 + 相似度分数)
  → WebView: 搜索结果列表，点击跳转到对应笔记
```

### 4. AI 完整管线（v1.0.0-alpha）
...
本地部署场景下，PostgreSQL + pgvector 太重；Qdrant 需要独立进程；LanceDB 偏向列式分析。Iris 使用 SQLite + sqlite-vec 在单文件内完成全文、向量、图三种检索。
