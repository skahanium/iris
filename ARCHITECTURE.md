...
密钥获取 fail-hard：凭证记录存在但解密/解析失败时直接报错，**绝不静默生成新密钥覆盖**——静默轮换会让既有快照永久不可读（历史事故根因）。快照不可读（`AppError::CasUnreadable`，如磁盘损坏或密钥缺失）不是自动删除依据。回收清单可标记便利副本 `unreadable`，但保留原版本身份与对象所有权；恢复不能把跳过便利副本等同于删除历史。`version_cleanup` 只按保留规则清理到期、未定稿的自动空闲快照，不因读取失败删除手动或定稿版本。未知归属历史同样不参与自动清理。旧非 CAS 快照仍属于兼容读取材料；其永久清理与故障恢复正在复审，不能声称所有历史都已转换为加密 CAS。

## SQLite 与迁移

当前共有 **75 组**增量迁移（`001` 至 `075`）。074 仅调整既有版本表的归属和约束，不删除用户历史；down 遇到旧结构无法表达的已知归属、回收归属或孤立历史时事务性拒绝，不通过筛掉记录完成回滚。075 追加 `audit_boundary_events` 诊断表，不是第二套 Run 生命周期。
