...
| Tool Calling | 支持 | 为插件系统的 Function Calling 预留 |
...
### 为什么不是 CRDT 实时同步
Iris 在 v1.0 前不做实时协作。CRDT（如 Yjs）的引入会让编辑器的状态管理复杂度翻倍，而 Iris 的核心用户场景是个人笔记。单用户场景下，ProseMirror 的 Transaction 系统已经足够。v1.0 后再评估是否引入 CRDT。
