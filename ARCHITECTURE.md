...
- `src-tauri/src/ai_runtime/`：Agent Task Runtime、任务规划、检索 broker、证据包、模型网关、工具确认与追踪。
...
sqlite-vec 是可选加速能力，默认构建可在不可用时保留非向量检索。v1.2.6 将统一向量模型、索引代际和 broker 降级语义；当前工作树不保留已替代的 RAG 规格文档。

## AI、网络与 Skills

Agent Task Runtime 是当前主 AI 架构：`agent_task_*` IPC、checkpoint、权限预检、工具确认、deliberation/verification 状态和可恢复暂停共同承载长任务。TaskPlan 负责把长任务输出组织成 Markdown-first 的对话流与临时 tab，高价值产物进入可保存的临时 tab，过程 tab 只用于长任务进度。
...
API Key 使用本地 AES-256-GCM 加密存储，不使用系统凭据服务。主密钥在平台配置目录，密文在应用数据目录；每条记录使用随机 12 字节 nonce 和服务名 AAD，解密值由 `Zeroizing` 持有。完整安全策略见 [SECURITY.md](./SECURITY.md)。

## SQLite 与迁移

截至当前基线共有 **46 组**增量迁移（`001` 至 `046`），每组都含 up/down 脚本。schema 变更只能新增迁移，不能要求用户删除数据库重建。依赖 sqlite-vec 的历史迁移按扩展可用性 best-effort 执行；标量表的可用性不依赖该扩展。
