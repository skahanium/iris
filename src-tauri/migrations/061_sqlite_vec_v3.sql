...
INSERT INTO vec_anchors_v3 (anchor_id, embedding)
SELECT anchor_id, embedding
FROM semantic_anchor_embeddings_v2
WHERE dimension = 512
  AND length(embedding) = 2048;

INSERT INTO vec_regulations_v3 (regulation_id, embedding)
SELECT regulation_id, embedding
FROM regulation_embeddings_v2
WHERE dimension = 512
  AND length(embedding) = 2048;
...
BEGIN
    INSERT INTO vec_anchors_v3 (anchor_id, embedding)
    VALUES (NEW.anchor_id, NEW.embedding);
END;
...
BEGIN
    ...
    INSERT INTO vec_anchors_v3 (anchor_id, embedding)
    SELECT NEW.anchor_id, NEW.embedding
    WHERE NEW.dimension = 512
      AND length(NEW.embedding) = 2048;
END;
...
BEGIN
    INSERT INTO vec_regulations_v3 (regulation_id, embedding)
    VALUES (NEW.regulation_id, NEW.embedding);
END;
...
BEGIN
    ...
    INSERT INTO vec_regulations_v3 (regulation_id, embedding)
    SELECT NEW.regulation_id, NEW.embedding
    WHERE NEW.dimension = 512
      AND length(NEW.embedding) = 2048;
END;
