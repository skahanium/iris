-- 072: domain capability bindings and run snapshots.
--
-- Rebuilds mcp_capability_bindings and agent_run_mcp_tool_snapshots so the
-- capability CHECK accepts both generic external.read and frozen domain
-- web.domain.read mappings. Existing rows are preserved as external.read with
-- no domain operation and an empty output mapping.

PRAGMA foreign_keys=OFF;

CREATE TABLE IF NOT EXISTS mcp_capability_bindings_new (
    id                      TEXT PRIMARY KEY,
    ...
    capability              TEXT NOT NULL CHECK (capability IN ('external.read', 'web.domain.read')),
...
INSERT INTO mcp_capability_bindings_new
    (id, provider_id, exposed_name, mcp_tool_name, input_schema_json,
     argument_mapping_json, output_policy_json, capability, domain_operation,
     output_mapping_json, risk_class, read_only, user_trusted,
     provider_config_hash, binding_config_hash, created_at, updated_at)
SELECT
    id, provider_id, exposed_name, mcp_tool_name, input_schema_json,
    argument_mapping_json, output_policy_json, capability, NULL,
    '{}', risk_class, read_only, user_trusted,
    provider_config_hash, binding_config_hash, created_at, updated_at
FROM mcp_capability_bindings;

DROP TABLE mcp_capability_bindings;
ALTER TABLE mcp_capability_bindings_new RENAME TO mcp_capability_bindings;

CREATE INDEX IF NOT EXISTS idx_mcp_capability_bindings_provider
    ON mcp_capability_bindings(provider_id, updated_at);

CREATE UNIQUE INDEX IF NOT EXISTS idx_mcp_domain_binding_provider_operation
    ON mcp_capability_bindings(provider_id, domain_operation)
    WHERE domain_operation IS NOT NULL;

CREATE TABLE IF NOT EXISTS agent_run_mcp_tool_snapshots_new (
...
INSERT INTO agent_run_mcp_tool_snapshots_new
    (run_id, binding_id, provider_id, exposed_name, mcp_tool_name,
     ...
     domain_operation, output_mapping_json, risk_class, read_only, user_trusted,
     provider_config_hash, provider_launch_hash, transport_kind,
     transport_config_json, credential_refs_json, binding_config_hash,
     frozen_at, snapshot_integrity_hash)
SELECT
    run_id, binding_id, provider_id, exposed_name, mcp_tool_name,
    input_schema_json, argument_mapping_json, output_policy_json, capability,
    NULL, '{}', risk_class, read_only, user_trusted,
    provider_config_hash, provider_launch_hash, transport_kind,
    transport_config_json, credential_refs_json, binding_config_hash,
    frozen_at, snapshot_integrity_hash
...
ALTER TABLE agent_run_mcp_tool_snapshots_new RENAME TO agent_run_mcp_tool_snapshots;
