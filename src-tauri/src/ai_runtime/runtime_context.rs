...
pub fn build_runtime_context_prompt(
    ...
    block.push_str(
        "\nVerification rule: local time is only a temporal reference. Historical assistant messages, summaries, and old citations are continuity aids, never independent evidence. For an external factual answer, use eligible web evidence returned by web_search; never invent a source or claim of verification.\n",
    );
    block
}
