...
pub(crate) fn truncate_web_context_text(text: &str, max_chars: usize) -> String {
...
pub(crate) fn initial_run_search_queries(query: &str) -> Vec<String> {
...
#[derive(Debug, Clone)]
pub(crate) struct SearchProviderFetch {
    pub(crate) body: String,
    pub(crate) search_backend: WebSearchBackend,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum SearchProviderCandidate {
    Mcp(String),
}

pub(crate) fn search_provider_candidates(
    db: &Database,
...
pub(crate) fn observe_mcp_search_provider_call(
    db: &Database,
    provider_id: &str,
    success: bool,
    elapsed: Duration,
...
pub(crate) fn ensure_provider_circuit_allows(provider_id: &str) -> AppResult<()> {
...
pub(crate) fn record_provider_failure(provider_id: &str) {
    crate::ai_runtime::circuit_breaker::record_failure(provider_id);
}

pub(crate) fn search_probe_counts_toward_circuit(diagnostic: &McpSearchResultDiagnostic) -> bool {
...
pub(crate) fn is_transient_provider_error(error: &AppError) -> bool {
...
pub(crate) fn mcp_runtime_failure_code(error: &AppError) -> &'static str {
    let message = error.to_string().to_ascii_lowercase();
...
pub(crate) fn sanitize_mcp_runtime_error(error: AppError) -> AppError {
...
pub(crate) fn resolve_mcp_provider_mapping(
    db: &Database,
    provider_id: &str,
    capability: &str,
    expected_snapshot: Option<
...
pub(crate) fn effective_mcp_search_mapping(
    db: &Database,
    provider_id: &str,
    mapping_json: &str,
) -> String {
...
pub(crate) fn build_mcp_fetch_arguments(
    mapping_json: &str,
    url: &str,
    max_chars: usize,
) -> serde_json::Value {
...
pub(crate) fn mcp_search_result_body(result: &serde_json::Value) -> String {
...
pub(crate) fn parse_freshness_label(text: &str) -> Option<String> {
...
pub(crate) fn web_evidence_items_from_search_fetch(
...
pub(crate) fn record_successful_search_usage(
...
pub(crate) fn parse_search_result_rows(body: &str) -> Vec<SearchResultRow> {
...
pub(crate) fn suppress_search_provider_failures_if_success(items: &mut Vec<WebEvidenceItem>) {
...
pub(crate) fn normalize_evidence_items(items: Vec<WebEvidenceItem>) -> Vec<WebEvidenceItem> {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum FetchProviderCandidate {
    Mcp(String),
    Native,
}

#[derive(Debug, Clone)]
pub(crate) struct PageProviderFetch {
    pub(crate) completeness: crate::llm::fetch_web_page::PageContentCompleteness,
    pub(crate) title: String,
...
pub(crate) fn fetch_provider_candidates(
    db: &Database,
...
pub(crate) fn is_allowed_page_fetch(
...
pub(crate) fn page_provider_fetch_from_native(page: PageFetchResult) -> PageProviderFetch {
    PageProviderFetch {
...
pub(crate) fn mcp_page_fetch_result(
    provider_id: &str,
    url: &str,
    result: &serde_json::Value,
) -> AppResult<PageProviderFetch> {
...
pub(crate) fn mcp_fetch_failure_kind(
    result: &serde_json::Value,
) -> Option<McpApplicationFailureKind> {
...
pub(crate) fn apply_page_provider_fetch(item: &mut WebEvidenceItem, page: PageProviderFetch) {
...
pub(crate) fn explicit_url_item(url: &str) -> WebEvidenceItem {
    if !is_https_url(url) {
        return failed_evidence_item(
            url,
            "native.url",
...
pub(crate) fn failed_evidence_item(
...
pub(crate) fn result_hash(parts: &[&str]) -> String {
