...
pub enum EmbeddingStartResult { Started, AlreadyRunning }

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum EmbeddingStartSource { Manual, Automatic }
...
impl EmbeddingBatcher for BgeEmbeddingBatcher {
    fn ensure_available(&self) -> AppResult<()> { super::engine::ensure_embedding_model_available() }
    fn embed_batch(&self, texts: &[&str]) -> AppResult<Vec<Vec<f32>>> { embed_texts_batch(texts) }
}
...
impl EmbeddingScheduler {
    ...
    pub fn with_batcher(db: Arc<Database>, batcher: Arc<dyn EmbeddingBatcher>) -> Arc<Self> {
        Arc::new(Self { db, batcher, runtime: Mutex::new(RuntimeState { foreground_busy: true, ..RuntimeState::default() }), app_handle: Mutex::new(None) })
    }

    pub fn attach_app_handle(&self, app_handle: AppHandle) {
        if let Ok(mut handle) = self.app_handle.lock() { *handle = Some(app_handle); }
    }

    pub fn status(&self) -> AppResult<EmbeddingIndexStatus> { self.db.with_read_conn(embedding_index_status) }

    pub fn enqueue_file(&self, _file_id: i64) {
        // Coverage is source-fingerprint based. A generation already running will
        // discover this change at its next short DB read; an idle generation waits
        // for the explicit automatic/manual start policy.
    }

    pub fn start_generation(self: &Arc<Self>, source: EmbeddingStartSource) -> AppResult<EmbeddingStartResult> {
        {
            let mut runtime = self.runtime.lock().map_err(|_| AppError::msg("Embedding scheduler lock poisoned"))?;
            if runtime.running { return Ok(EmbeddingStartResult::AlreadyRunning); }
            if source == EmbeddingStartSource::Automatic && (!runtime.initial_index_complete || runtime.foreground_busy || runtime.manual_paused) {
        ...
        if let Err(error) = transition {
            if let Ok(mut runtime) = self.runtime.lock() { runtime.running = false; }
            return Err(error);
        }
        ...
        thread::Builder::new().name("iris-embedding-scheduler".into()).spawn(move || scheduler.run_generation())
            .map_err(|error| AppError::msg(format!("Failed to start embedding scheduler: {error}")))?;
        Ok(EmbeddingStartResult::Started)
    }

    pub fn mark_initial_index_complete(self: &Arc<Self>) {
        let epoch = match self.runtime.lock() {
            Ok(mut runtime) => { runtime.initial_index_complete = true; runtime.activity_epoch },
            Err(_) => return,
        };
    ...
    pub fn set_foreground_busy(self: &Arc<Self>, busy: bool) {
        let epoch = match self.runtime.lock() {
            Ok(mut runtime) => { runtime.foreground_busy = busy; runtime.activity_epoch = runtime.activity_epoch.wrapping_add(1); runtime.activity_epoch },
            Err(_) => return,
        };
        if !busy { self.schedule_auto_start(epoch); }
    }
    ...
    pub fn set_manual_paused(self: &Arc<Self>, paused: bool) -> AppResult<()> {
        let should_resume = {
            ...
            !paused && !runtime.foreground_busy && runtime.initial_index_complete && !runtime.running
        };
        if paused {
    ...
    fn schedule_auto_start(self: &Arc<Self>, epoch: u64) {
        let scheduler = Arc::clone(self);
        let _ = thread::Builder::new().name("iris-embedding-idle".into()).spawn(move || {
            thread::sleep(IDLE_DELAY);
            let allowed = scheduler.runtime.lock().is_ok_and(|runtime| {
                runtime.activity_epoch == epoch && runtime.initial_index_complete && !runtime.foreground_busy && !runtime.manual_paused && !runtime.running
            });
            if allowed { let _ = scheduler.start_generation(EmbeddingStartSource::Automatic); }
        });
    }
    ...
    fn run_generation(self: Arc<Self>) {
        ...
        if let Err(_) = result {
            let _ = self.db.with_conn(|conn| mark_failed(conn, "model_unavailable", "Embedding model unavailable"));
            self.finish_worker();
            return;
        }
        loop {
            ...
            let batch = match self.db.with_read_conn(load_pending_batch) {
                Ok(batch) => batch,
                Err(_) => { let _ = self.db.with_conn(|conn| mark_failed(conn, "database_error", "Embedding database unavailable")); self.finish_worker(); return; }
            };
            if batch.is_empty() {
                let completion = self.db.with_conn(finalize_if_covered);
                if completion.is_err() { let _ = self.db.with_conn(|conn| mark_failed(conn, "database_error", "Embedding database unavailable")); }
                self.emit_status();
                self.finish_worker();
                return;
            }
            let texts = batch.iter().map(|record| record.text.as_str()).collect::<Vec<_>>();
            let vectors = match self.batcher.embed_batch(&texts) {
                Ok(vectors) if vectors.len() == batch.len() && vectors.iter().all(|vector| vector.len() == EMBEDDING_DIMENSION) => vectors,
                _ => { let _ = self.db.with_conn(|conn| mark_failed(conn, "embedding_failed", FAILED_SUMMARY)); self.emit_status(); self.finish_worker(); return; }
            };
            if self.db.with_conn(|conn| commit_batch(conn, &batch, &vectors)).is_err() {
                let _ = self.db.with_conn(|conn| mark_failed(conn, "database_error", "Embedding database unavailable"));
                self.emit_status(); self.finish_worker(); return;
            }
            self.emit_status();
            thread::yield_now();
        }
    ...
    fn should_pause(&self) -> bool { self.runtime.lock().map(|runtime| runtime.manual_paused || runtime.foreground_busy).unwrap_or(true) }
    fn finish_worker(&self) { if let Ok(mut runtime) = self.runtime.lock() { runtime.running = false; } }
    fn emit_status(&self) {
        let Ok(status) = self.status() else { return; };
        if let Ok(handle) = self.app_handle.lock() { if let Some(handle) = handle.as_ref() { let _ = handle.emit("embedding-index-progress", status); } }
    }
}

#[derive(Clone, Copy)]
enum RecordKind { Chunk, Anchor, Regulation }
struct PendingRecord { kind: RecordKind, id: i64, text: String, fingerprint: String }
...
pub fn embedding_index_status(conn: &Connection) -> AppResult<EmbeddingIndexStatus> {
    ...
    match status { Ok(Some(status)) => Ok(status), Ok(None) => Ok(legacy_ready_status()), Err(error) if unavailable_schema(&error) => Ok(legacy_ready_status()), Err(error) => Err(error.into()) }
}
...
fn transition_running(conn: &Connection, source: EmbeddingStartSource) -> AppResult<bool> {
    ...
    if matches!(status.phase.as_str(), "running" | "paused") { return Ok(false); }
    if source == EmbeddingStartSource::Automatic && (status.phase != "legacy_ready" || status.automatic_attempted) { return Ok(false); }
    if !matches!(status.phase.as_str(), "legacy_ready" | "failed" | "ready") { return Ok(false); }
    conn.execute(
...
fn load_pending_batch(conn: &Connection) -> AppResult<Vec<PendingRecord>> {
    ...
    if records.len() < BATCH_SIZE { records.extend(missing_records(conn, RecordKind::Anchor, BATCH_SIZE - records.len())?); }
    if records.len() < BATCH_SIZE { records.extend(missing_records(conn, RecordKind::Regulation, BATCH_SIZE - records.len())?); }
    Ok(records)
}

fn missing_records(conn: &Connection, kind: RecordKind, limit: usize) -> AppResult<Vec<PendingRecord>> {
    ...
    let rows = stmt.query_map(rusqlite::params![EMBEDDING_MODEL_ID, EMBEDDING_DIMENSION as i64, (EMBEDDING_DIMENSION * std::mem::size_of::<f32>()) as i64, limit as i64], |row| Ok(PendingRecord { kind, id: row.get(0)?, text: row.get(1)?, fingerprint: row.get(2)? }))?;
    let _ = query;
...
fn commit_batch(conn: &Connection, batch: &[PendingRecord], vectors: &[Vec<f32>]) -> AppResult<()> {
    ...
    for (record, vector) in batch.iter().zip(vectors) {
        let (source_sql, table, column) = match record.kind { RecordKind::Chunk => ("SELECT COALESCE(content_hash, '') FROM chunks WHERE id = ?1", "chunk_embeddings_v2", "chunk_id"), RecordKind::Anchor => ("SELECT COALESCE(content_hash, '') FROM semantic_anchors WHERE id = ?1", "semantic_anchor_embeddings_v2", "anchor_id"), RecordKind::Regulation => ("SELECT COALESCE(content_hash, '') FROM regulation_index WHERE id = ?1", "regulation_embeddings_v2", "regulation_id") };
        let current: Option<String> = transaction.query_row(source_sql, [record.id], |row| row.get(0)).optional()?;
        if current.as_deref() != Some(record.fingerprint.as_str()) { continue; }
...
fn total_sources(conn: &Connection) -> AppResult<i64> { Ok(conn.query_row("SELECT (SELECT COUNT(*) FROM chunks) + (SELECT COUNT(*) FROM semantic_anchors) + (SELECT COUNT(*) FROM regulation_index)", [], |row| row.get(0))?) }
fn valid_sources(conn: &Connection) -> AppResult<i64> {
    ...
    Ok(conn.query_row(sql, rusqlite::params![EMBEDDING_MODEL_ID, EMBEDDING_DIMENSION as i64, bytes], |row| row.get(0))?)
}

fn legacy_ready_status() -> EmbeddingIndexStatus { EmbeddingIndexStatus { active_model_id: LEGACY_MODEL_ID.into(), target_model_id: EMBEDDING_MODEL_ID.into(), dimension: EMBEDDING_DIMENSION as i64, phase: "legacy_ready".into(), indexed_items: 0, total_items: 0, last_error: None, failure_code: None, automatic_attempted: false } }
fn unavailable_schema(error: &rusqlite::Error) -> bool { matches!(error, rusqlite::Error::SqliteFailure(_, Some(detail)) if detail.contains("no such table")) }
...
mod tests {
    ...
    fn unknown_vector_metadata_does_not_count_as_coverage() {
        ...
        conn.execute("INSERT INTO chunk_embeddings_v2(chunk_id,embedding) VALUES (1, zeroblob(2048))", []).unwrap();
