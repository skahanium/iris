...
> **N · Notion** 灰阶：圆角方框 + 斜切衬线大写「I」；全平台同一套 SVG 几何。无渐变、无墨迹手写。
>
> UI 气质：**冷灰壳层 + 低饱和冷调鼠尾草绿品牌点**（`--brand` / `--knowledge-accent`，hue ~108）。Monogram 本身保持灰阶；彩色品牌点只用于交互语义（wiki、rail 激活、知识选中），不涂在 logo 上。
...
| `--brand` / `--knowledge-accent` | `108 12% 58%` | `108 14% 42%` |

### 品牌点使用边界

| 可用                                                                  | 禁止                                                         |
| --------------------------------------------------------------------- | ------------------------------------------------------------ |
| wiki 链接、rail/outline 激活、overlay 选中、tip callout、知识图谱强调 | logo / monogram 填色、正文大面积染色、紫/霓虹渐变、装饰 glow |
| `--ai-mention` 浅绿色内联名称                                         | mention 胶囊、底色块、图标装饰                               |
