...
- 文档与 Agent 默认解耦。仅当普通文档存在非空文字选区且 Agent presentation 可见时，才在 Composer 内部显示使用 `surface-inset`、品牌前缘和淡入淡出的紧凑「当前选区」上下文条；候选不是持久化引用，也不会因为打开文档、切换会话或浏览正文而自动产生。
...
- 发送瞬间冻结 ready 候选与其引用，选区失焦、折叠或更新不会改写已开始的 Run；Run 接受后才解除候选。用户消息保留只读引用条，当前轮显示内存截断预览，历史轮次仅显示安全文件名标记，不写入选区正文。
...
- 普通域 Composer 以 TipTap 原子 `assistantMention` 节点承载 `@` 文档/文件夹和 `#` 标签。Suggestion 负责多词、Unicode 空白、中文括号与 IME 查询；复制/粘贴文本不会恢复授权，涉密域不创建 mention。发送前统一投影为自然纯文本与 UTF-16 `DisplayMention.range`，普通域和涉密域草稿按内存快照隔离。
