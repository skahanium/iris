...
- 文件夹在文件之前，沿用 `buildVaultTree` 的 `zh-CN` 排序。
- 展示 Markdown 笔记和当前 `workspaceList` 已支持的媒体；保留路径分类和 reserved root 过滤。
- 文件行显示类型图标、用户可见标题、锁定状态；不显示绝对 vault 路径。
- 当前文档自动展开祖先文件夹、滚动显露并使用 brand marker 标记。
- 目录展开集合以 vault identity 隔离持久化，切换 vault 不串用。
