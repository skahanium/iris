...
| Agent Run         | `assistant_run_start`、`assistant_run_control`、`assistant_run_confirmation_diff`、`assistant_run_get`、`assistant_run_diagnose`        | 按安全域路由的执行、控制、确认差异预览、状态读取与 C27 诊断查询；normal-domain 从 C26 解释，classified 报告持久缺口而不是空成功 |
...
- `assistant_run_confirmation_diff` 按 `session + runId + confirmationId + planHash` 返回待确认冻结计划的**有界统一 diff**（逐目标 hunks、行级增删与上下文、截断标志）。响应是面向用户本人复核的瞬时预览：不写入持久化事件、工具结果、审计或日志；持久化事件与工具结果仍只投影 `summary` + `targets`，不含笔记正文、工具参数或 diff。确认事件、`planHash` 失配返回 `agent_run_confirmation_plan_hash_mismatch`；无 pending 确认或无法复放候选返回 `agent_run_confirmation_diff_unavailable`；过期返回 `agent_run_confirmation_expired`。classified 域不提供差异预览（`agent_run_control_not_available`），涉密 Run 沿用现有易失流程。预览不授予、不延长、不改写变更计划；批准仍只授权冻结时的不可变参数。
...
- `mcp_capability_bindings_list(providerId?)`、`mcp_capability_binding_upsert(input)`、`mcp_capability_binding_delete(bindingId)`：管理稳定 binding。Upsert 会再次 discovery、复核声明与 provider 配置 hash，并要求 `userTrusted: true` 表示用户已对精确 provider/tool/schema 完成二次确认；renderer 不能仅凭服务端声明自行证明只读。新 binding 一律是逐 Run 的通用 `external.read`，不接受 `domainOperation` 或 `outputMapping`。列表响应仍可能携带这两个已退役字段，用于展示和删除历史 binding；它们不会进入新 Run 的授权面。
