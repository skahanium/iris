...
依据：[讨论纪要](../AGENT-REFORM-DISCUSSION-2026-09-14.md)、[开发规范](../AGENTS.md) 与源码基线 `2670739f`。2026-09-16 用户确认 `R11`（约束分不变量／信封／配方，模型拥有下一步）为决定；同日确认 `R12`（完成额度预留是输出帽信封，不是关面配方）。§3 M5、§7.4、§10.3、§11 按上述决定修订。旧 Harness 体系已经归档，本文不继承其阶段完成度；历史入口仍由 [文档索引](./README.md) 管理。当前实现描述结合 [ARCHITECTURE.md](../ARCHITECTURE.md) 与源码核对。仓库仍规定 [ROADMAP.md](../ROADMAP.md) 承载版本排期，但其中旧 Agent 路线与归档阶段不能直接作为本次革新的执行依据；本文第 10 节明确迁移依赖与承接责任，不等待旧排期来决定技术先后，也不宣告旧文档整体废止或阶段已经完成。
...
拥有循环过程和统一预算账本。模型负责语义计划、修正建议，以及观察之后的下一步提议。控制器执行信封（授权、预算、取消）与不变量（权限、单一权威状态、不重放未知副作用）；不得靠配方启发式当第二规划器（`R11`）。完成额度预留是输出帽信封，预留触达后可不提供业务工具，但不得注入关面指令（`R12`）。

| 组件               | 输入 → 输出                                                                     | 决定权与不变量                                                                                                                 | 当前基础及处置                                                                                                                                                                   |
| ------------------ | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C13 循环状态与预算 | Run 合同、动作申请、用量 → 执行阶段、额度租约、剩余资源                         | 额度只有一个权威账本；派发、内部请求、token、时间和子任务分维度记录；类别耗尽不封死无关能力；完成额度预留是输出帽信封（`R12`） | [agent_tool_loop.rs](../src-tauri/src/ai_runtime/agent_tool_loop.rs)、RunBudgetPolicy；账本与信封；轮次上限是配置不是架构真理                                                    |
| C14 行动与恢复协调 | 模型提议、观察、验证缺口 → 修复反馈、信封内停止理由，或将模型下一步提议交给派发 | 无效提议不当作执行；不给无信息增益的相同错误无限重试；结束必须对应真实原因；下一步由模型提议；预留触达不写关面指令（`R12`）    | 当前循环、[run_engine/recovery.rs](../src-tauri/src/ai_runtime/run_engine/recovery.rs)；反馈与信封内停止；禁止配方强制综合                                                       |
...
目标预算口径与当前父、子循环各自限额的做法不同：C13 保留父子层级明细，同时控制顶层总消耗，原生搜索辅助模型请求也纳入。数值先沿受控配置，属于信封而非架构真理，不在本文凭直觉扩大；调整时必须兼容旧 Run 存档，旧确认不扩权。
...
对可恢复的提议或调用错误，在授权仍有效、用户未取消且资源允许时，默认至少提供一次带具体反馈的纠偏机会。C13 在可调用工具的 Run 中为纠偏和最终表达预留资源，不能正常行动耗尽全部模型回合后才发现无法纠正；不足以满足该合同的配置应明确标为受限配置。完成额度预留触达后可不提供业务工具，但必须把 remaining 留给模型自己的答案，不得注入配方关面指令（`R12`）。
...
| C20 双路网页搜索协调 | 模型提出的查询、外发批准、两路能力、额度 → 两路状态与合并线索   | 每次获准的新搜索，两条可用路线都真实执行；原生不支持时正常单 MCP；不能伪造另一侧结果 | [dual_path_search.rs](../src-tauri/src/ai_runtime/dual_path_search.rs) 已接入生产 `web_search`。实现事实见 [K12](../agent-harness/contracts/K12-native-search-subrequest.md) 执行事实与 [G02](../agent-harness/requirements/open-items.md) 现状；`G02` 仍 open，不是 `V04`／`V05`。 |
...
多步、状态与撤回拆开，不得互相株连（`R11`）：

- **多步（N21）**：观察返回后，只要信封允许，模型仍可再提议工具。这是语义，不是步骤清单。
- **续跑（N22）**：本次不承诺普通研究 Run 在进程重启后从任意中间点续跑；保留已有安全会话恢复与受限变更检查点语义。只有 Durable Run 具有暂停与检查点。未来扩大执行恢复能力需单独明确，不隐藏在“有记忆”这一名称里。
- **撤回（N23）**：用户撤回已发布答案不等于 Host 在循环内替模型重规划。
...
| F04 读取时修复并写库：C18、C04、C26 | 直到 `058366ea`，[mcp_runtime_registry.rs](../src-tauri/src/ai_runtime/mcp_runtime_registry.rs) 的 `list_web_evidence_providers` 仍调用 heal 并 UPDATE；该提交后改为只读列举。兼容补字段发生在显式 `upsert`，运行时 overlay 不写库。时序见 `Q05`。 | 普通列举不得触发隐藏配置变更。目标保持：配置升级在明确的保存边界；查询保持只读；既有信任与快照不能随新哈希自动扩权。关闭仍等 `D04`／`Q05` 验收，不因本源码修正宣布双路已接通                                                                                  |
...
F04 的 list 写库直到 `058366ea` 才从现行源码消除（落点见 `Q05`），不是 `145bc563`／`70c929ac`：[mcp_external_tools.rs](../src-tauri/src/ai_runtime/mcp_external_tools.rs) 仍比较绑定哈希与当前 provider 哈希，不一致时拒绝。显式保存仍可改哈希并要求重新审查；普通列举不得换哈希。是否曾导致某次用户故障仍需关联运行证据。配置内容实际变化后重算哈希本身是必要的；`Q05` 仍由 `D04` 验收关闭。
...
| D1 修正协议与有界纠偏     | C11／C12、C13／C14、C17／C23 | 依赖 D0 的证据口径；先覆盖 F03、未知工具纠正、参数修正和工具后续轮，证明停止原因与错误反馈到达正确边界；过早收束负例与进展身份（G06／N15／N21）成立后，再将新循环路径作为默认 |
...
- 约束分不变量、信封、配方三层；配方不是架构真理。观察之后下一步由模型提议；Host 不得靠配方当第二规划器。正权与安全同级。完成额度预留是输出帽信封，预留触达后可不提供业务工具，但不得注入关面指令（`R12`）。
...
- 第二套循环内核或 Skill 执行器；把配方写进检查器死规则。
