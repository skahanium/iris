...
依据：[讨论纪要](../AGENT-REFORM-DISCUSSION-2026-09-14.md)、[开发规范](../AGENTS.md) 与源码基线 `2670739f`。2026-09-16 用户确认 `R11`（约束分不变量／信封／配方，模型拥有下一步）为决定，§3 M5、§7.4、§10.3、§11 按该决定修订。旧 Harness 体系已经归档，本文不继承其阶段完成度；历史入口仍由 [文档索引](./README.md) 管理。当前实现描述结合 [ARCHITECTURE.md](../ARCHITECTURE.md) 与源码核对。仓库仍规定 [ROADMAP.md](../ROADMAP.md) 承载版本排期，但其中旧 Agent 路线与归档阶段不能直接作为本次革新的执行依据；本文第 10 节明确迁移依赖与承接责任，不等待旧排期来决定技术先后，也不宣告旧文档整体废止或阶段已经完成。
...
拥有循环过程和统一预算账本。模型负责语义计划、修正建议，以及观察之后的下一步提议。控制器执行信封（授权、预算、取消）与不变量（权限、单一权威状态、不重放未知副作用）；不得靠配方启发式当第二规划器（`R11`）。

| 组件               | 输入 → 输出                                                                     | 决定权与不变量                                                                               | 当前基础及处置                                                                                                                                                                   |
| ------------------ | ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C13 循环状态与预算 | Run 合同、动作申请、用量 → 执行阶段、额度租约、剩余资源                         | 额度只有一个权威账本；派发、内部请求、token、时间和子任务分维度记录；类别耗尽不封死无关能力  | [agent_tool_loop.rs](../src-tauri/src/ai_runtime/agent_tool_loop.rs)、RunBudgetPolicy；账本与信封；轮次上限是配置不是架构真理                                                    |
| C14 行动与恢复协调 | 模型提议、观察、验证缺口 → 修复反馈、信封内停止理由，或将模型下一步提议交给派发 | 无效提议不当作执行；不给无信息增益的相同错误无限重试；结束必须对应真实原因；下一步由模型提议 | 当前循环、[run_engine/recovery.rs](../src-tauri/src/ai_runtime/run_engine/recovery.rs)；反馈与信封内停止；禁止配方强制综合                                                       |
...
- 约束分不变量、信封、配方三层；配方不是架构真理。观察之后下一步由模型提议；Host 不得靠配方当第二规划器。正权与安全同级。
