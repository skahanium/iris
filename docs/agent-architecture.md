...
| C20 双路网页搜索协调 | 模型提出的查询、外发批准、两路能力、额度 → 两路状态与合并线索   | 每次获准的新搜索，两条可用路线都真实执行；原生不支持时正常单 MCP；不能伪造另一侧结果 | [dual_path_search.rs](../src-tauri/src/ai_runtime/dual_path_search.rs) 已接入生产 `web_search`；生产原生 `unsupported`，MCP 单路是正常配置。`G02` 仍 open，双路未接通          |
