...
| F04 读取时修复并写库：C18、C04、C26 | [mcp_runtime_registry.rs](../src-tauri/src/ai_runtime/mcp_runtime_registry.rs) 的 `list_web_evidence_providers` 已改为只读列举，不再在读取路径 UPDATE 映射、哈希与时间；兼容补字段发生在显式 `upsert`，运行时 overlay 不写库 | 普通列举不得触发隐藏配置变更。目标保持：配置升级在明确的保存边界；查询保持只读；既有信任与快照不能随新哈希自动扩权。关闭仍等 `D04`／`Q05` 验收，不因本源码修正宣布双路已接通                                                                                  |
...
F04 的 list 写库已从现行源码消除：[mcp_external_tools.rs](../src-tauri/src/ai_runtime/mcp_external_tools.rs) 仍比较绑定哈希与当前 provider 哈希，不一致时拒绝。显式保存仍可改哈希并要求重新审查；普通列举不得换哈希。是否曾导致某次用户故障仍需关联运行证据。配置内容实际变化后重算哈希本身是必要的；`Q05` 仍由 `D04` 验收关闭。
