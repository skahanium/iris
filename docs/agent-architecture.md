...
| C14 行动与恢复协调 | 模型提议、观察、验证缺口 → 下一轮行动、修复反馈或结束理由   | 无效提议不当作执行；不给无信息增益的相同错误无限重试；结束必须对应真实原因；下一步由模型提议 | 当前循环、[run_engine/recovery.rs](../src-tauri/src/ai_runtime/run_engine/recovery.rs)；反馈与信封内停止；禁止配方强制综合                                                       |
