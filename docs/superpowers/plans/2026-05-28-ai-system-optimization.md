...
test.describe('AI 工作流验收', () => {
  ...
  test('工具调用显示', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="tool-call-bubble"]')).toBeVisible();
  });
});
```
...
```tsx
import { ContextPacket, EvidenceRelation, EvidenceRelationType } from '@/types/ai';
...
export function EvidenceChainView({ packets, relations }: EvidenceChainViewProps) {
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {packets.map((packet) => (
          <div key={packet.id} className="rounded-lg border p-3">
            ...
            <p className="mt-1 text-xs text-muted-foreground">{packet.excerpt}</p>
          </div>
        ))}
      </div>
      ...
      {relations.length > 0 && (
        <div className="space-y-2">
          <h5 className="text-xs font-medium text-muted-foreground">关联关系</h5>
      ...
      <div className="space-y-2">
        {plan.steps.map((step, index) => (
          <div key={index} className="flex items-center gap-2 text-xs">
            ...
            <span className="text-muted-foreground truncate">{step.query}</span>
          </div>
        ))}
      </div>
...
export function InlineSuggestion({
  ...
  return (
    <div className="absolute z-50 mt-1 p-2 bg-popover border rounded-lg shadow-lg max-w-md">
