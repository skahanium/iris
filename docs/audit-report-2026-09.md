...
### 建议处置表（2026-09-24 补录，处置状态截至该日）

| #   | 建议                                 | 处置          | 依据／对应提交                                                                                               |
| --- | ------------------------------------ | ------------- | ------------------------------------------------------------------------------------------------------------ |
| 1   | 脚注定义编辑丢失（P0-1）             | 未处理        | 编辑器保真债，列待办；历波明确不做 Markdown 保存改造                                                         |
| 2   | 显式保存往返改写（P0-2）             | 未处理        | 同上（「Markdown 保存 P0」为历波明确不做项）                                                                 |
| 3   | N17 文案回正（E5）                   | 未处理        | 待办                                                                                                         |
| 4   | 差异预览失败禁批（E1）               | 已采纳·已修复 | `71731801`（approveDisabled 门闩＋默认展开）；`ec77d76e` 补批准门闩键与服务端边界对齐（F9）                  |
| 5   | 过程轨迹合并失败覆盖（E2）           | 未处理        | 待办                                                                                                         |
| 6   | Inline AI 边界合同澄清               | 未处理        | 待办（先文档/决定登记）                                                                                      |
| 7   | 图片索引跳围栏（E3）＋F01 收清       | 部分采纳      | F01 写语义已收（`web_search_route_promote`＋两设置入口交替保存 V04 场景）；E3 未处理                         |
| 8   | V05 五组 live＋来源 UI 分渠道        | 部分采纳      | V05 窄窗口 2/13 端点已实跑（2026-09-23，有界承诺已落档）；E6 未投影——与「UI 状态投影缺位」同族，待新 UI 工作 |
| 9   | unknown 强制差异交付＋触发改显式操作 | 部分采纳      | 强制冻结＋差异（`71731801`）＋不确定项交付（`ec77d76e`）；触发改 Intake 操作类型未做                         |
| 10  | Q06/Q07 子任务参数消费               | 未处理        | D06 前置，D06 未开工                                                                                         |
| 11  | Q13 预算存档兼容                     | 未处理        | 待办                                                                                                         |
| 12  | 前端维护性长线项                     | 未处理        | 长线                                                                                                         |
