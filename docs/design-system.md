...
| **Editor canvas** | 居中内容栏约 `52rem`，与背景同色  | 扉页衬线标题 + 无衬线正文、两端对齐         |
...
- 编辑区：`max-width: 52rem`，水平 `clamp(1.5rem, 5vw, 6rem)`，正文 `16px` / `line-height: 1.62`，段落**两端对齐**
...
`ManagementCenterPanel` 是唯一管理型入口，使用横向全宽顶部标签导航，一级分区固定为 `总览 / 笔记 / 知识库 / AI`。取消左侧设置栏、工作台页、系统安全页和关于页；系统边界、许可和源码信息并入总览。总览、知识库默认不设置子页面，内容多时在同页纵向滚动；笔记允许使用同窗子页承载文件树与回收站等笔记库管理工作流；AI 允许使用同窗子页承载模型、联网、人格、Skills 和记忆等复杂配置，子页必须保留返回顶层分区的入口。模型配置拆为供应商连接、模型目录与能力路由三层：供应商级测试只验证凭据和端点，模型级验证负责文本/视觉能力确认，Ollama 不出现在外部供应商设置面板。
