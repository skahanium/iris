...
AI 会话流式显示采用“权威答案 + 按帧 reveal”的双层模型：`presentation.answer` 始终保存完整权威内容，前端按帧释放小段可见文本，避免 provider 一次给出大段 delta 时整段重排造成闪烁。消息列表在最新内容下方保留固定底部留白，自动跟随滚动到滚动内容底部；只有用户明确向上离开底部一段距离后才解除跟随，回到底部附近时自动恢复跟随。脱离跟随期间，底部水平居中显示圆形向下箭头按钮（无文字，带 `aria-label="回到最新"`），点击后回到最新输出并恢复跟随。reveal 尚未结束时，即使 Run 已完成，气泡仍保持流式渲染直到可见文本追平。
