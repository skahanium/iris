...
主模型在尚未产生可见输出、工具调用或 continuation 时切换备用，过程区显示「已切换到备用模型」；`web.search` 或 `web.fetch` 在冻结路由内切换时显示「联网检索服务暂不可用，正在尝试另一服务」。两者均使用中性可访问状态，不显示 provider endpoint、模型凭据、搜索词、URL 或原始错误。模型已有可见动作后不再跨 Provider 自动切换；Web 工具切换不暗示模型发生变化。

强时效 Run 的最低取证由 Host 在首个模型答复前执行，过程区只能如实显示「正在联网检索」或「正在读取页面」；它不是模型提出的工具调用，也不得展示为“模型已搜索”。未派发的工具提议不显示抓取、失败或切换状态。
