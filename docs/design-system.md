...
AI 气泡轻分层：助手近透明弱边；用户 `--ai-user-bg` 为极浅 brand tint；处理过程与来源区共用同一套紧凑的消息元信息折叠栏骨架（相同图标、文字层级、左右内边距与弱分隔线），仅以顶部/底部位置区分执行过程和证据披露；折叠摘要末项在正文目标确认且本地播放完成后为「答复完毕」，追赶期间为「正在显示答复」。同一回答的重复联网搜索在过程栏合并成单项并累计耗时，避免把同一能力的多次调用展示为冗余步骤。用户气泡以内容 `fit-content` 收缩包裹，并保留对消息行可用宽度 88% 的上限；短中文消息不得因任意断词规则而逐字折行，长文本、URL 与代码仍可在上限内安全换行。默认可见的 AI 侧栏必须在 Vault 选择屏期间预热其独立模块；首帧只初始化可交互壳层，MCP binding discovery 等非必要信息在首帧之后再请求。历史与当前答复共用 Markdown 块/行窗口，普通长段落复用已确认分片；语法高亮延后并限定片段规模。复杂活动块的解析成本需纳入桌面性能验收。

AI 会话流式显示采用“权威答案 + 局部呈现”的双层模型。会话视图身份只在主动新建、加载会话或切换安全域时重置；intake 回执补齐标识不重建滚动区。可见文本按实际帧时间和字素边界释放，默认约 4 个视觉行/秒，积压时连续追赶至最多 20 行/秒；连续换行每次最多推进一行。远端完成只结束生成，不跳过本地播放，也不回退已经显示的正文。追赶期间可立即显示全部；发送下一问题先补齐已完成回答。流式与完成态共用 Markdown 块和正文容器，标题、列表、围栏和表格的活动结构连续更新；已完成块只在视口与预渲染区域外回收成实测高度占位，长回答不截去原文。

对话滚动由单一阅读锚点控制器处理。新轮次动态补足底部空间，把用户消息停在顶部 12px 附近；随后答案向下生长，接近至少 96px 的底部留白后连续跟随。用户上滚优先并中断自动运动，脱离期间保持可见块及块内位置；回到底部或点击带 `aria-label="回到最新"` 的圆形箭头恢复跟随。尺寸观察覆盖字体、图片、输入框和面板变化，不重复测量同一行。`prefers-reduced-motion: reduce` 关闭人为播放和滚动动画，仍保持正文身份与位置稳定。真实桌面帧预算、长文和事件乱序验收是发布证据，jsdom 测试不替代桌面体验验证。

处理过程只展示实际发生且必要的高层动作，不展示查询正文、工具参数或内部推理。续答失败的阶段表述遵守下文「AI 能力降级状态」，不得凭搜索调用推断整个检索任务已经完成。
...
- **文末**：助手消息正文下方固定可折叠 **「来源」** 区块（`AssistantCitationFooter`）。精确绑定时默认收起并显示自然来源类别计数，例如“用户输入 1 · 授权材料 2 · 网页 2 · 推断 1”；展开后只列出最终正文实际引用、且已成功登记的 `citation_map` HTTPS 来源：`序号 · 标题`（可点击打开系统浏览器）。现代消息以持久化的最终 evidence 选择为唯一投影事实：显式空选择即使该 Run 曾检索或抓取也不显示来源；非空选择只显示被最终正文采用的来源。只有缺少选择字段的旧消息可使用 **「本次检索来源」** 兼容读取，且展开说明仅表示历史检索范围、**不表示已逐段核验**。受控来源区是所有回答的唯一来源清单：正文不得额外生成“资料来源 / 参考来源 / Sources / References”附录、原始 URL 清单或“来源见下方”。无可验证证据或回答降级为证据不足说明时，不显示来源区或模型拼接的链接。所有 Agent 入口在终态事务成功前不得显示候选正文；净化、来源绑定与修复在发布前结束，正常路径不发送 `AnswerReset`。正文来源附录在确认唯一终稿前识别并剥离，普通正文保持完整；不得先显示伪精确 `[Wn]` 标记，也要把模型写出的“用户提供”归因或“本轮 / 上一轮”生命周期措辞改为自然中性表达，不得在成功终局时整段回放。不展示 snippet、搜索词、工具参数、原始摘录或内部推理。
...
底部状态栏右侧不再设置通用活动文字区：不展示 Agent 处理过程、工具活动、空闲文案或笔记库同步数量，也不保留对应的分隔符、悬浮提示和播报节点。Agent 过程只在对话回答内展示。
...
主模型在尚未产生可见输出、工具调用或 continuation 时切换备用，过程区显示「已切换到备用模型」；`web.search` 在冻结 MCP 路由内改用备用时显示「搜索已改用备用服务」，`web.fetch` 显示「网页读取已改用备用服务」。同一回答中同一能力的重复切换合并为一条过程提示，完整切换事件仍保留；没有对应能力而被跳过的候选不算服务失败。`native.fetch` 不是 MCP 路由赢家，不得显示检索服务切换。两者均使用中性可访问状态，不显示 provider endpoint、模型凭据、搜索词、URL、工具名或原始错误。模型已有可见动作后不再跨 Provider 自动切换；Web 工具切换不暗示模型发生变化。守线核验未取得正文时，过程区显示「未取得可核验网页正文」，不得与误报切换文案同时出现。
