...
AI 会话流式显示采用“权威答案 + 按帧 reveal”的双层模型：`presentation.answer` 始终保存完整权威内容，前端按帧释放小段可见文本，避免 provider 一次给出大段 delta 时整段重排造成闪烁。消息列表在最新内容下方保留固定底部留白，自动跟随滚动到滚动内容底部；用户向上滚动即解除跟随，并显示「回到最新」。reveal 尚未结束时，即使 Run 已完成，气泡仍保持流式渲染直到可见文本追平。
