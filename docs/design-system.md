...
- **导航器视觉**：标题行使用“笔记库”、快捷键和图钉；图钉仅保存固定偏好，宽度不足时必须以 tooltip 说明仍会显示为浮动抽屉。标题下方使用紧凑、无业务卡片感的 icon 工具条承载根目录新建笔记、根目录新建文件夹、全部展开和全部折叠；所有 icon-only 控件都有中文可访问名称与 tooltip。不得为此加入目录内文字筛选、排序、计数、拖放或永久三栏。
- **目录树层级**：文件夹以展开/收起图标表达状态，文件使用文档图标；嵌套项目沿真实祖先分支绘制细的冷灰连续导轨。当前文件仅使用弱 brand tint、brand 前缘 marker 与文字，不使用厚边框或分类色；用户选择“全部折叠”后，同一当前文件不得立即反弹展开，只有切换当前文件才自动显露其祖先。
