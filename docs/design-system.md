...
图片粘贴／拖入绑定发起时的文档基线与选区；等待下载期间的普通编辑只映射原选区，切换／重新加载文档则停止迟到插入。下载失败用现有轻量提示说明可重试，不重播原剪贴板事件，也不在新文档中补插旧内容。
...
AI 会话流式显示采用“权威答案 + reveal”的双层模型：`presentation.answer` 始终保存完整权威内容。可见文本按当前行剩余宽度释放，当前行先向右长，满行后再换行；空行每帧最多填约四成行宽，避免窄栏一次跳出多行。正在输出的 assistant 气泡钉在虚拟列表外的底部真 DOM，不参与行变换回收。消息列表在最新内容下方保留固定底部留白；新一轮先把用户气泡停在视口上方附近，随后答案在视口内向下长，只有顶到留白才按溢出平移画布，不得每增高一次就把 `scrollTop` 钉到内容底。只有用户明确向上离开底部一段距离后才解除跟随，回到底部附近时自动恢复跟随。脱离跟随期间，底部水平居中显示圆形向下箭头按钮（无文字，带 `aria-label="回到最新"`），点击后跟随溢出钉住最新输出，不重新把长答案停回用户气泡顶部。回车发送时同一帧先提交用户气泡，输入框在 layout 阶段再清空，避免 TipTap 先塌高度。reveal 尚未结束时，即使 Run 已完成，气泡仍保持流式渲染直到可见文本追平。`prefers-reduced-motion: reduce` 时整段放出。

当联网检索已完成但模型 Provider 不可用时，错误提示明确说明检索已完成、失败发生在模型阶段；不得把它显示成搜索失败或丢失上下文。处理过程只展示实际发生且必要的高层动作，不展示查询正文、工具参数或内部推理。
...
- 联网开关表示持久化用户授权，服务发现失败或暂时不可用不得自动关闭；可用性单独显示。已授权时可按需核实公共事实，不逐题重复申请联网。处理过程统一使用中文动作名（包括「读取网页」），未知工具显示通用中文动作，技术 ID 仅用于诊断。
- 笔记变更确认必须展示 Host 冻结的真实 Markdown 差异、创建／移动路径、回收目标及全部链接影响；摘要不能替代差异。确认前等待目标在途保存，基线改变时重新预览。后台变更回执与编辑器脏修订冲突时保留编辑内容并展示冲突，禁止重存旧正文。
- 「撤销本次 Agent 变更」与编辑器撤销、版本恢复、会话撤回分别命名；前者以实际执行回执生成反向计划并重新确认，不由模型重写。移动部分成功时先接纳已移动事实，再显示未完成影响面，不提示重复移动。
- 版本面板按笔记库、文档会话与路径绑定异步结果；切换目标后不得应用迟到的预览或恢复。旧历史无法确定原笔记库时标注「归属待确认」，恢复确认必须写明当前目标路径；未关联路径的旧历史仅在用户主动展开「未归属历史」后列出，不自动认领或删除。恢复读取期间再编辑正文时停止应用并提示重新预览。

- 补充信息只用于没有该字段就无法安全执行的硬前置条件；能通过限定范围、披露地域或说明不确定性继续回答的问题不得中断 Run。天气、附近影院和明确场次等本地可用性请求可要求城市，宽泛的近期发行、推荐和综述直接按有界 Web 研究回答。
- `InputRequired` 必须绑定到发起它的 Run，并在该轮 assistant 位置显示紧凑的对话内卡片；禁止在 Agent 顶部、全局横幅或 Composer 之外另设悬浮输入面板。字段由 Run 合同提供，界面不得为单一领域维护平行状态。
- 卡片支持字段填写、回车或「继续」提交和「取消本轮」。提交后卡片原位退出，过程提示立即切换为「正在恢复运行状态」，随后由真实阶段事件覆盖；取消后的历史孤立用户轮次显示「本次回答已取消」。
- 恢复继续使用同一 `runId`、turn、用户问题与已校验结构化输入。会话历史若只有该 Run 的用户行，投影层须在其后幂等补建唯一 assistant 行；无对应用户行的迟到事件仍必须忽略。终态正文、过程与引用只从同 Run 持久化消息补齐，禁止为恢复显示而重跑 Provider。
...
- **文末**：助手消息正文下方固定可折叠 **「来源」** 区块（`AssistantCitationFooter`）。精确绑定时默认收起并显示自然来源类别计数，例如“用户输入 1 · 授权材料 2 · 网页 2 · 推断 1”；展开后只列出最终正文实际引用、且已成功登记的 `citation_map` HTTPS 来源：`序号 · 标题`（可点击打开系统浏览器）。现代消息以持久化的最终 evidence 选择为唯一投影事实：显式空选择即使该 Run 曾检索或抓取也不显示来源；非空选择只显示被最终正文采用的来源。只有缺少选择字段的旧消息可使用 **「本次检索来源」** 兼容读取，且展开说明仅表示历史检索范围、**不表示已逐段核验**。受控来源区是所有回答的唯一来源清单：正文不得额外生成“资料来源 / 参考来源 / Sources / References”附录、原始 URL 清单或“来源见下方”。无可验证证据或回答降级为证据不足说明时，不显示来源区或模型拼接的链接。严格来源路径在验证绑定前不得显示临时正文，修复成功只出现一次最终正文，失败不以 `AnswerReset` 撤回重写。所有流式与终局使用同一净化结果：疑似尾部来源标题先暂存，确认是列表才剥离，正常正文则原样释放；不得先显示伪精确 `[Wn]` 标记，也要把模型写出的“用户提供”归因或“本轮 / 上一轮”生命周期措辞改为自然中性表达，不得在成功终局时整段回放。不展示 snippet、搜索词、工具参数、原始摘录或内部推理。
- **正文表达**：来源和核验边界由来源区承载；普通回答自然陈述事实、分析与不确定性，不展示 `Run`、`current_run_web`、`[Wn]`、来源组协议、或“本轮 / 上一轮已核验”分类。普通回答不得开场播报本题是否需要联网/搜索/工具、是否为主观分析，或宣布“直接拆解”。用户直接 `@` 选择的材料在来源摘要中归为“授权材料”，自动取得的本地笔记归为“本地检索”；两者都不是用户原话。仅当用户明确询问来源、核验过程或不确定性时，才用自然语言说明“我暂未查到可靠来源”等限制，仍不得泄露内部协议。
...
- **应用工作区抽屉**：RSS 等应用工作区的来源抽屉必须从 `--titlebar-height` 下方开始，不得侵入 macOS traffic lights；中宽度由 Sheet 外框提供唯一右边界，窄窗口占满可用工作区且不保留右侧空白轨。抽屉内导航必须占满容器并移除自身右边框，禁止重复分隔线。
...
主模型在尚未产生可见输出、工具调用或 continuation 时切换备用，过程区显示「已切换到备用模型」；`web.search` 或 `web.fetch` 在冻结 MCP 路由内改用备用时显示「已改用备用检索服务」。`native.fetch` 不是 MCP 路由赢家，不得显示检索服务切换。两者均使用中性可访问状态，不显示 provider endpoint、模型凭据、搜索词、URL、工具名或原始错误。模型已有可见动作后不再跨 Provider 自动切换；Web 工具切换不暗示模型发生变化。守线核验未取得正文时，过程区显示「未取得可核验网页正文」，不得与误报切换文案同时出现。

`WebRequired` 的最低取证由 Host 在首个模型答复前执行，过程区只能如实显示「正在联网检索」或「正在读取页面」；它不是模型提出的工具调用，也不得展示为“模型已搜索”。日常 `WebPreferred` 不走该 Host 起步。未派发的工具提议不显示抓取、失败或切换状态。
...
- 管理中心按缓存域显示占用、条目数和策略；使用既有 `SettingRow`、`StatusValue` 与中性 `Button`，不新增专用视觉语法。
- “安全清理”只覆盖可重建的 RSS 媒体、网页正文与临时文件；更新包与运行时资源显示“受保护”。运行时资源必须通过单独“修复”动作，并明确提示下次重启才会重建。
