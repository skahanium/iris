...
Key，也不会连接模型或 MCP 端点。启动预检子进程前还会移除名称类似 key、
token、secret、password、authorization、credential 的环境变量。
...
不进入报告。匿名 ID 必须来自同一次预检；缺少 ID、伪造 ID或不在本次预检中的
ID 都会在创建临时状态和派发前被拒绝。

批准后，选中的非密钥路由与 MCP 元数据只能复制到 `tempfile` 管理的独立
`AppState`。审批本身仍不能把状态提升为 live 通过，pilot 上限固定为 12 题。
当前尚未取得具体匿名 profile 的批准，因此 `live pilot` 仍停在显式费用
checkpoint，不会发起网络请求。取得批准后，12 题必须走与 deterministic
评测相同的 normal headless 路径；任何进一步承压扩展还需再次确认费用。
