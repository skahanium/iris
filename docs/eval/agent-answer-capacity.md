...
| 栏            | 关键指标                                                                                       | 门槛                                                            |
| ------------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| quality       | 事实 Precision 10000 / Recall 6666 / F1 7999；必需来源召回 6666；引用支持 6666；约束遵循 10000 | 事实召回与引用支持门槛未过；约束遵循门槛通过                    |
| hardAdmission | 授权违规 0；Offline Web 泄漏 0；高风险无依据主张 12                                            | `zeroToleranceGate=false`（12 个隐式 vault 失败记为无依据主张） |
...
- `npm run audit:rust` / `npm run docs:check`

`npm audit` 仍报告既有前端传递依赖告警（brace-expansion / dompurify /
js-yaml / linkify-it），非本轮新增。真实 MiniMax/AnySearch pilot 因出站
不可达保持 `live_not_tested`；确定性分栏 scorecard、13 维压力轴与 pairwise
能力矩阵已写入版本化报告。
