...
> 结论：**未达到发布或完成条件。** 当前分支新增了统一 Run 的基础合同、Repository 和部分命令，但新旧执行系统仍并存，生产前端尚未切换，且数据迁移、领域行为、涉密会话和发布验证均未满足规范。
...
| 8 前端切换             | 发送仅调用 `assistant_run_start`；单 reducer；无 editor 绑定     | `UnifiedAssistantPanel.impl.tsx` 仍使用 `useAssistantTasks`/`useAssistantHarnessResume`；`assistantRunStart` 没有生产调用                                     | 未完成   | 重建 run controller 和 session hooks，移除 TaskPlan/scene/旧恢复 UI，inline AI 仅传 explicitAction  |
