...
| AAR-009 | 已修复并增强             | DeliberationState 和 verification summary 已持久化并透出到 UI。             |
...
| AAR-022 | 已修复                   | approve、reject、deny、timeout 都进入统一 audit。                           |
...
- verification 未通过时不弹窗、不硬阻断普通回答，只在回答末尾和任务事件里写极短提示；预算或轮次耗尽仍保持可恢复暂停。
