...
> **证据可复现性提醒（2026-09-13 核对）**：本文件引用的 `/tmp/iris-*.log` 是一次性本地日志，`/tmp` 不跨重启保留，文中 9 个日志路径现已全部不存在。这些日志只是**当时**的先行失败/通过记录，不能作为当前可复核证据；需要证据时按文中列出的测试与探针命令重新执行，并在记录中附上日期、构建与执行人。
...
| 读取未获准 URL 导致整轮失败  | `web_url_not_public_https`（非公开 HTTPS）与 `tool_not_in_run_surface`（工具或 URL 不在本轮允许面）在派发前反馈，模型可改查再读取；真实失败转为生产回归 |
