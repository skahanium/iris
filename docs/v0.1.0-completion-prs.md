...
本文档记录 **初版脚手架合并之后**，为达到 [ROADMAP.md](../ROADMAP.md) 中 v0.1.0「核心功能 + 验收标准」所需的最小 PR 切片。

- **初版施工切片**（地基 → 编辑器 → AI → 搜索）：见 [v0.1.0-epic.md](./v0.1.0-epic.md)
- **差距审计**（2026-05）：初版约 60–70% 可演示 MVP；下列 PR 用于闭合缺口
...
## 刻意不纳入本清单（v0.1.1 / v0.2）

| 项 | 说明 |
|----|------|
| sqlite-vec 虚拟表 | 工作量大；PR-C8 文档对齐即可 |
| 正文 `#tag` 索引与聚合 | v0.2 标签系统 |
| 完整 GFM（脚注、数学公式等） | 非 v0.1 硬门槛 |
| 自定义 API base URL 表单 | 可挂在 PR-C2 stretch（`settings` 存 `custom_base_url`） |
| Playwright 全链路 E2E | v0.1 用手工清单 + 占位 Vitest |

---

## 发版前复检清单

- [ ] ROADMAP 五条验收标准各有演示步骤或测试/文档证据
- [ ] `pnpm tauri dev`：选目录 → CRUD → 外部改文件 → 提示重载
- [ ] 内联 AI：接受 / 重试 / 回退各测一次
- [ ] AiPanel：提问可见关联笔记上下文
- [ ] 语义搜索：Top-5 评测记录（`docs/eval/semantic-search.md`）
- [ ] 笔记目录、SQLite、日志无 API Key 明文
- [ ] `cargo clippy -D warnings` 与前端 lint/typecheck/test 全绿
...
维护说明：合并 PR-C* 后可在本表更新「状态」列，并在 PR-C10 同步 [ROADMAP.md](../ROADMAP.md) 复选框。
