...
### PR-C7 — `feat(settings): 联网检索凭据 UI`

**阻塞**：LLM Key + 联网检索 Key

| 项   | 内容                                                                    |
| ---- | ----------------------------------------------------------------------- |
| 范围 | 设置页、`credential_*` IPC、`search_web.rs`                             |
| 实现 | MiniMax Token Plan Key（`iris.minimax`）；未配置或失败时降级 DuckDuckGo |
| Done | 设置页可配置 MiniMax；无 Key 仍可用 DuckDuckGo                          |
