...
每个 operation 在进入软件门禁和实例门禁前，必须有已确认的 PDR（见 [`07-provider-landing-and-decision-process.md`](07-provider-landing-and-decision-process.md)）。没有 PDR 的 operation 不得标记为 Ready/Operational，也不得进入受支持 operation 软件门禁计数。
...
## 4. 受支持 operation 软件门禁（目标 11 个）

每个测试必须通过正式 intake 创建 Run，冻结本地 contract fixture（结构化路由为 MCP/REST snapshot；News WebFallback 为 WebEvidenceBroker fixture），登记真实数据库 evidence ID，由 Host/结构化 finalization 生成最终消息，并从数据库恢复。

> 以下 11 个为目标清单；若决策门 6 选择缩减支持矩阵，则只要求当前支持矩阵内的 operation 全部通过，未支持的 operation 不进入计数。

```text
production_weather_current_uses_frozen_provider
production_weather_forecast_uses_frozen_provider
production_news_search_uses_frozen_provider_or_web_fallback
...
- operation、工具名和路由一致；若走结构化 snapshot，则 snapshot 完全一致；若 news 走 WebFallback，则断言 WebEvidenceBroker 路径而非 MCP snapshot；
- Provider/binding/config hash 属于 Run 接受时冻结事实（结构化路由）；
...
- 无 binding：Unconfigured，工具不进入 surface（以决策门 2 确认为准；若选择“可发现但不可用”，也必须保持 fail-closed）。
...
当前实例必须导出受支持 operation 的 readiness（目标 11 个），并同步到 [`06-instance-readiness-record.md`](06-instance-readiness-record.md)。每个受支持 operation 至少有一个 Ready Provider 覆盖其声明支持范围；随后人工执行（仅当前支持矩阵内的项）：
...
> 结构化领域软件门禁 N/N 通过（目标 11/11）；当前实例受支持 operation readiness 全部为 Operational；天气、新闻、金融、娱乐和体育真实场景均完成，未发生普通 Web 冒充、证据身份混淆或敏感诊断泄漏。
