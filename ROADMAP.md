...
- 标题栏增加可见的笔记库导航入口。默认以左侧浮动抽屉展示上下分层导航：上层只定位文件夹，下层只展示选中文件夹的直属文件；打开文档后保持抽屉，支持连续浏览；抽屉不参与正文与 Agent 的宽度分配。鼠标点击文件树、Tab、更多和新建控件不显示焦点光晕，键盘聚焦保留可见提示。
...
- 轻量导航器以紧凑 Obsidian 式上下浏览器呈现：标题行仅显示“笔记库”；上层提供当前目录新建文件夹、目录排序与展开/折叠，只显示文件夹及直属 Markdown 数；下层提供媒体开关、当前目录搜索、文件排序与新建笔记，只显示直属文件。两层以可拖动 45/55 分隔线连接，保持冷灰 + brand 绿，不恢复永久三栏。
