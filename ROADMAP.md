...
- HR-3 已验收：Direct 继续保持 1 次模型、0 工具；普通 ToolLoop 使用 8 次模型、24 次工具总上限，并冻结 local 12、network 6、external-read 6、runtime 4、confirmed-change 6 的分类上限。Web、本地检索、runtime 和外部只读工具共享同一个 `AgentToolLoop`；相同成功调用不重复，相同失败最多两次，连续两轮无新资源或内容、探索额度耗尽或只余最终模型回合时关闭工具并保留最后一次综合。旧 Web 专用 `FreshResearchPlan`、`EvidenceGap`、平行搜索/抓取/修复计数与 deadline 已退役；历史领域 envelope 只保留兼容读取路径。
...
- 2026-08-31：HR-6 已删除领域 tool catalog、dispatcher、Host 预取与专用终局路径；migration 072、旧 envelope 与旧 snapshot 仅兼容读取，活跃旧领域 Run 会在 Provider 调度前安全终态化。HR-7 的确定性矩阵已覆盖 24 个通用场景、八项重复硬边界与 OpenAI-compatible/Anthropic mock 续接；真实 Provider 质量、延迟和成本试点仍须用户另行授权，不能由 fixture 结果代替。
...
- 验收区分合同单测、行为集成、回答质量和真实 Provider 试点。确定性矩阵必须覆盖普通对话、差结果自动改写查询、本地多跳、混合资料、严格事实、chat-only Provider、冻结写入和恢复；不固定唯一查询顺序，也不以历史通过数证明当前语义质量。真实试点必须另行授权模型、成本和场景，核心不得按 MiniMax、MiMo 或其他模型名称分叉。
