...
- 证据型回答通过内部结构化终局提交按内容块绑定当前 Run 的 `U/M/L/W/E/T/H/I` 来源；Harness 校验后自动投影行内 Web 引用、归因与来源摘要，模型不能伪造最终映射。普通未校准内容可按“本次检索来源”来源组展示且不宣称逐段核验；涉及当前日期、天气、新闻、金融、影视可用性和体育赛况的严格事实时，来源组不能代替实体、数值、日期、地域与数据时点的确定性支持，终局协议不可用或证据不足必须失败关闭。文末来源区默认最小可见地显示来源类别计数，展开后才显示经过本轮引用过滤的 HTTPS 标题列表；不展示摘录、查询、工具参数或内部推理。
