...
- 正文核心：本地打包 Noto Sans SC；编辑态取消两端对齐；canvas 消费 `--prose-measure`；wiki（brand）≠ 外链（primary）；抬升 light 下 code/callout 对比。

### Wave 2 — 节奏、动效与 chrome

- 正文节奏：measure/行高/块距/标题阶梯；callout 语义色收官；导出 HTML 与编辑面审美对齐。
