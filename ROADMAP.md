...
**Segment 1 验收（Home / 品牌轨 / 空主面，已交付）：** 冷启动有笔记时自动打开；关光 Tab 显示 WorkspaceEmpty 且不自动打开；库空显示 VaultEmpty（`workspace-empty` vault 模式）；Iris 品牌轨纯标识、不可点击；打开失败在空主面展示可读错误。人工清单见 [iris-rail-refresh-manual-checklist](./docs/testing/iris-rail-refresh-manual-checklist.md)。

**Segment 2 验收（Agent 气泡 / Composer / 过程文案，已交付）：** 用户与助手气泡轻分层可辨；发送与主操作 CTA 使用 `variant="brand"`；过程区在最终正文开始流式输出后折叠，完成摘要为「答复完毕」；历史轮次可重新展开安全过程。人工清单见 [iris-rail-refresh-manual-checklist](./docs/testing/iris-rail-refresh-manual-checklist.md)。

**Segment 3 验收（正文节奏与对比，已交付）：** `--prose-measure` 与编辑态 `text-align: justify` 保持硬锁；标题阶梯与块距消费 prose token；亮色 code/callout 对比抬升；编辑区与会话 Markdown 共用 `markdown-prose.css` 审美（无独立「导出 HTML」产品面）。

**Segment 4 验收（壳层收敛 + 文档，已交付）：** 顶栏、底栏、AI 侧车外层分隔与 Overlay 顶栏统一消费 `border-subtle` 与 chrome 字号阶梯（`text-caption`/`text-micro`）；Rail Tab 激活与 Outline marker 对齐 `--brand`，不改 ghost 几何；人工清单含壳层亮暗抽检。

- Token：`--brand`、边框三级、warning/success 表面、chrome 字号阶梯；合并 rail；亮色补全全部 `--status-*`；联网光带仅用 token。
- 组件强制消费语义色与 `shadow-overlay`/`shadow-floating`；业务面禁止 `amber-*`/`emerald-*` 与默认 `shadow-sm`/`shadow-md`。
- UX 真相：接通 AI activity → Composer/StatusBar；修正空态文案；Skills/会话历史中文化；Outline 无标题时保持空白；默认隐藏 provider 诊断条。
- 正文核心：本地打包 Noto Sans SC；编辑态正文两端对齐；canvas 消费 `--prose-measure`；wiki（brand）≠ 外链（primary）；抬升 light 下 code/callout 对比。

### Wave 2 — 节奏、动效与 chrome

- 正文节奏：measure/行高/块距/标题阶梯；callout 语义色收官；编辑与会话共用 prose token 审美对齐。
- 真实浮层 enter/exit 动效（挂 `--motion-*`）；统一焦点环；过程点尊重 `prefers-reduced-motion`。
- Chrome 字号/边框收敛；AI 冷加载 skeleton；Tooltip 原语；Overlay/搜索 AbortController；人工清单与「命令面板已退役」事实同步。
