...
- HR-7 产品门已拆成 `agent:eval:contract`、一次确认的双路 4 Run `agent:eval:live canary`、通过后一次确认的双路 12 Run `agent:eval:live campaign` 和最终 `agent:eval`。INC-HR-008 将旧 live v2 的 404/fixture 语义 oracle、单路 24/18 平均预算和不可审阅失败工件替换为 v3：两条匿名路由交替执行并共享全局 12 Run、48 次模型调用、36 次 Web 工具调用；报告只证明终态、授权、search→fetch、Run-local 引用、安全和连续性，最终可见回答与有限来源摘录写入忽略目录的签名审阅包。产品门要求两份不同 `routeCommitment`、同一 campaign 的 v3 轨迹、审阅包哈希与四维人工评分（单项至少 4/5、平均至少 4.2）同时通过。确定性演练不等于真实质量；INC-HR-007 的旧 MiniMax/MiMo 运行结论不可复用，新的双路复测与人工审阅尚未执行，因此 HR-7 仍未通过。
