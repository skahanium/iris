...
Iris 采用里程碑式版本规划。当前开发基线为 **v1.2.6**；本文件是版本排期的唯一来源。`ARCHITECTURE.md` 只描述已存在的结构，`CHANGELOG.md` 只记录已交付的变化。
...
## v1.2.6 — RAG 检索可靠性与中文质量（进行中）

目标是在不破坏既有 Markdown、搜索和 AI 工作流的前提下，完成可测量的检索基线升级。
...
完整设计、数据迁移、验收门槛及非目标见 [docs/specs/v1.2.6-rag-optimization.md](./docs/specs/v1.2.6-rag-optimization.md)。
