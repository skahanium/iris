...
**状态**: 已于 2026-05-25 发布（见 [CHANGELOG.md](./CHANGELOG.md)）。下列 `[x]` 与 PR-C1～C9 及仓库测试/文档一致。
...
- [ ] 自定义 OpenAI 兼容 **API Base URL** 设置界面（IPC 字段 `custom_base_url` 已预留）→ **v0.1.1**
- [ ] Playwright / WebDriver **全链路 E2E**（v0.1 以 Vitest 场景占位 + 手工验收为准）→ **v0.1.1**
- [ ] **sqlite-vec 虚拟表**语义索引 → **v0.2**「搜索与索引 · 语义检索升级」
...
- [ ] 全应用组件扫尾：对话框 / Quick Open / 搜索面板统一 chrome 层级
- [ ] 设置区收纳 API Key（减少侧栏表单感，仍内置非插件）

### 功能与质量

- [ ] 自定义 OpenAI 兼容 **API Base URL** 设置界面
- [ ] Playwright / WebDriver **全链路 E2E**（可选）

### 验收标准

- [ ] 默认启动为「深灰外壳 + 暖纸编辑区」，与 design-system 截图级一致
- [ ] 无紫色主色；AI 面板收起后编辑区可视宽度明显增加
- [ ] `npm run lint` / `typecheck` / `test` 与 Rust CI 保持通过
...
- [ ] **`[[双向链接]]`**：支持 `[[笔记标题]]` 语法，自动维护链接关系
- [ ] 反向链接面板（显示所有链接到当前笔记的笔记）
- [ ] **知识图谱可视化**：基于链接关系的力导向图
- [ ] 标签系统：`#标签` 语法，标签聚合视图
- [ ] 链接自动补全（输入 `[[` 时弹出笔记列表）
- [ ] 统计面板：笔记数量、链接数、标签数、写作字数趋势

#### 体验与界面（纸墨 · 阶段 1）

与 [design-system 阶段 1](docs/design-system.md#落地阶段与路线图版本对照) 同步，且服从知识网络信息架构：

- [ ] **引用卡**完整形态（章节 meta、「仅此次」、移除引用）— 对齐 ARCHITECTURE 线框
- [ ] **关联笔记**以上下文芯片展示，不采用聊天气泡主视觉
- [ ] **`/` 命令菜单**纸墨样式（非系统默认下拉）
- [ ] **反向链接 / 图谱 / 标签聚合**：chrome 外壳 + 纸面或中性面板，不引入高饱和装饰
...
- [ ] **sqlite-vec 集成**：`vec0` 虚拟表（或等价 API）存储 chunk 嵌入，维度与 v0.1 模型一致（AllMiniLML6V2 → 384 维）
- [ ] **增量 migration**：自 `chunk_embeddings` BLOB 回填/双写虚拟表；保留 BLOB 作回退或校验，直至稳定后可选删除
- [ ] **`search_semantic` 改造**：优先 SQL/扩展向量 Top-K；chunk 规模低于阈值时可保留 Rust 余弦路径
- [ ] **性能验收**：万级 chunk 下语义查询 P95 明显优于 v0.1 全表扫描（具体阈值在实现 PR 中定义）
...
- [ ] **笔记目录加密**：可选 AES-256-GCM 加密，应用启动时输入密码解密
- [ ] 文件外部修改冲突解决：三方 diff 视图，用户抉择保留哪个版本
- [ ] **版本记录系统**：
...
- [ ] 笔记模板系统：会议纪要、读书笔记、项目复盘等模板库
- [ ] 文件导出：Markdown、PDF、HTML 格式导出
- [ ] 图片拖拽插入/粘贴，本地图片管理

#### 体验与界面

- [ ] 版本时间线、diff、模板选择器沿用纸墨 token（编辑区不被「工具 UI」抢戏）
