...
- 轻量导航器以紧凑 Obsidian 式上下浏览器呈现：标题行仅居中显示“笔记库”；上层提供当前目录新建文件夹、目录排序与展开/折叠，只显示文件夹及直属 Markdown 数；下层提供媒体开关、当前目录搜索、文件排序与新建笔记，只显示直属文件。两层以可拖动 45/55 分隔线连接，保持冷灰 + brand 绿，不恢复永久三栏。
