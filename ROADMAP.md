...
当前视觉系统刷新目标为 **Iris Rail 完整界面系统**：常驻品牌轨道、Rail Segments Tab、沉静编辑画布、边缘目录轨、AI 协作侧车、任务舱 Overlay 与 AI 系统中心。完整设计见 [Iris Rail 完整界面系统刷新设计](docs/superpowers/specs/2026-06-09-iris-rail-refresh-design.md)。
...
- Skills 面板；Token 累计与工具状态在底栏（`StatusBarTokenUsage`）；证据统计在 `ContextPacketDrawer` 标题行
...
- `Ctrl+S` 层 1 保存；`Ctrl+Shift+S` 手动版本（`kind=manual`）；空闲 10 分钟 `auto_idle`、定稿当前正文
...
- E2E：Vitest 场景测试（v1.1.0 目标：Playwright 全链路 + 覆盖率 > 80%）
...
下列版本号已归档，不再独立维护。对应功能已并入 v1.1.0。
