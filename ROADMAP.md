...
**Skills 与插件的边界**：Iris 只支持由 Iris 对话生成、用户确认全文与 scope 后启用的 prompt-only `SKILL.md` 行为包，默认保存到当前 Vault。Skills 不是外部安装平台，也不承载 MCP、workspace、resource、脚本或依赖安装能力；它们不能扩展 UI、不能注册新节点类型、不能执行任意代码。

**扩展方式**（仅此几种）：

- **主线功能**：新能力通过版本里程碑交付
- **Skills 提示词包**：Iris 生成草稿，用户确认后保存为 prompt-only `SKILL.md`，见管理中心 `AI -> Skills`
...
- 图片与链接：`ImageExtension`、PM 热路径 `![alt](src)` 往返；`![[path]]` 支持图片 / 视频 / PDF 内嵌预览并只在可见 surface 申请媒体 lease；`EditorImageDropExtension` 拖放/粘贴写入 `assets/`（`vault_asset_write`）；`LinkExtension` 支持 `Ctrl+K` / `Cmd+K`
...
- 推理能力采用内置能力目录 + 验证补充 + 用户确认覆盖；验证模型只补充文本 / 视觉 / 推理来源，不把目录已知能力降级为未知
- 长上下文策略（`long_context`）、动态 token 预算
- DeepSeek 前缀缓存纪律（分层 messages、同会话同参数）
- 底栏 LLM/联网连通性指示器（`ConnectivityIndicators`）
- WebEvidenceBroker 统一联网语义层：联网搜索只通过显式映射的 MCP Provider；不内置原生或厂商搜索引擎；助手面只保留 `web_search`，搜索、抓取、URL 深读和证据输出都经 broker 进入现有证据包
...
- Quick Open（`Ctrl+P`）与文件树可打开笔记、图片、视频和 PDF；媒体文件作为独立只读 tab 预览，使用 opaque `iris-media` lease，关闭或切换时释放；全库搜索（`Ctrl+Shift+F`）、管理中心（`Ctrl+,` / 底栏齿轮）
- 文件树直达（`Ctrl+Shift+E` / `Cmd+Shift+E`）进入管理中心 `笔记 → 浏览笔记库`
- 回收站位于管理中心 `笔记` 分区；文件导出（Markdown / HTML）
- 笔记模板系统（内置 4 个 + 自定义 `.iris/templates/*.md`）
- 新建文档自动命名与标题同步：默认占位路径（如 `未命名文档.md`、`untitled-*`）在标题确定后跟随标题生成真实 `.md` 文件名；用户手动改过的自定义文件名不被标题覆盖；顶栏「+」固定在 Iris 品牌轨之后、tab rail 之前，不能漂到窗口控件旁。

### 界面系统

- **设计方向**：Notion 式扁平编辑（Inter、蓝灰 accent、无行线/纸页）
- 文档标题字段复用无衬线标题栈并启用 lining nums，避免中英混排标题里的数字高低不齐
...
- E2E：Vitest 场景测试（v1.2.1 目标：Playwright 全链路 + 覆盖率 > 80%）
- 42 个数据库 migration（001_core 到 042_web_evidence_provider_registry，含 up/down；sqlite-vec 相关迁移为 best-effort/optional）
...
下列版本号已归档，不再独立维护。对应功能已并入 v1.2.1。
