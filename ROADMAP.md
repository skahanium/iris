...
| **v0.3.1-ui** | （仅体验） | **命令浮层**、纸页视口、暗纸、圆角/动效、Chrome+AI 抛光 — 见 [design-system 阶段 1.5](docs/design-system.md#落地阶段与路线图版本对照) |
| **v1.0.0** | 稳定与合规 | **纸墨阶段 2～3**（可选）：Zen、标签栏隐藏；WCAG 与高对比主题 |
...
- [x] **信纸编辑**：行线、强制标题行（frontmatter `title`）、段首缩进、tab 同步、章节折叠、阅读时长 — 见 [design-system](docs/design-system.md)
- [ ] **体验收尾**（纸墨阶段 2～3，按需）：标签栏自动隐藏 — Zen 已随信纸交付（`Ctrl+.`）
- [ ] 自动更新：应用内更新检测和增量更新
- [ ] 完整的单元测试和端到端测试覆盖
