...
### v0.1.0 未纳入（延至 v0.1.1 或更晚）

- [ ] 自定义 OpenAI 兼容 **API Base URL** 设置界面（IPC 字段 `custom_base_url` 已预留）
- [ ] Playwright / WebDriver **全链路 E2E**（v0.1 以 Vitest 场景占位 + 手工验收为准）
- [ ] **sqlite-vec 虚拟表**语义索引（规划见 v0.2「搜索与索引 · 语义检索升级」）
...
**目标**: 开放扩展能力，让社区贡献 AI 命令、主题、集成。

### 核心功能

- [ ] **插件 API**：定义插件生命周期（加载、启用、禁用、卸载）
- [ ] AI 命令插件接口（注册自定义 `/` 命令）
- [ ] 编辑器扩展插件接口（添加自定义 TipTap 节点、Mark）
- [ ] 主题插件接口（自定义 CSS 变量覆盖）
- [ ] **社区插件市场**：插件的浏览、安装、更新、评价
- [ ] 插件沙箱：限制插件对文件系统和网络的访问权限

### 验收标准
- [ ] 第三方可以开发并发布一个自定义 AI 命令插件
- [ ] 插件安装/卸载不影响核心编辑器稳定性
- [ ] 插件权限声明清晰可见
...
以下功能不在里程碑承诺范围内，但值得在未来版本中评估：

- 移动端（iOS / Android）—— Tauri 2.x 移动端支持在 2026 年达到 GA 时考虑
- 实时协作（CRDT 同步，类似 Yjs）
- AI 自动标签和分类
- 语音转文字笔记
- 手写笔迹支持
...
v0.1.0 初版施工与**补齐 PR 清单**见 [docs/v0.1.0-epic.md](./docs/v0.1.0-epic.md)、[docs/v0.1.0-completion-prs.md](./docs/v0.1.0-completion-prs.md)。
