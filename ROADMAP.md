...
- 标题栏增加可见的笔记库导航入口。默认以左侧浮动抽屉展示单列真实目录树，打开文档后保持抽屉，支持连续浏览；抽屉不参与正文与 Agent 的宽度分配。
...
- 轻量导航器以紧凑 Obsidian 式目录呈现：标题行提供固定偏好图钉，工具条提供根目录新建、全部展开/折叠；文件夹图标和细树形导轨表达层级。保持冷灰 + brand 绿，禁止加入目录内筛选、排序、计数或永久三栏。
