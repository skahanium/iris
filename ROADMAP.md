...
- Run 需要不可缺少的结构化补充信息时，输入卡必须出现在该轮 assistant 位置，不使用侧栏顶部横幅；提交后以同一 Run 从 `preparing` 重新调度，直接消费已校验字段而不从拼接文本二次猜测。仅有用户历史行时幂等补建唯一 assistant 投影，完成态从持久化消息恢复，取消轮次明确标记且不自动重放。
...
- 证据型回答通过内部结构化终局提交按内容块绑定当前 Run 的 `U/M/L/W/E/T/H/I` 来源；`Wn` 是 Run-local Web 顺序号，`E/L` 使用 Iris ledger ID，`Mn` 是本轮授权材料顺序号，会话 `[Cn]` 只用于展示。`ProvenancePolicy` 是引用语法、Run 所有权和逐块覆盖的唯一校验事实源；Harness 校验后自动投影行内 Web 引用、归因与来源摘要，模型不能伪造最终映射。普通未校准内容可按“本次检索来源”来源组展示且不宣称逐段核验；涉及当前日期、天气、新闻、金融、影视可用性和体育赛况的严格事实时，来源组不能代替实体、数值、日期、地域与数据时点的确定性支持，终局协议不可用或证据不足必须失败关闭。当前门禁不宣称已经实现自由文本 NLI 语义核验。文末来源区默认最小可见地显示来源类别计数，展开后才显示经过本轮引用过滤的 HTTPS 标题列表；不展示摘录、查询、工具参数或内部推理。
- 联网证据默认首次检索 8 条、每次回答累计最多注册 12 条；后续检索只可使用剩余额度。20 秒总时限、32k 工具结果包、逐条摘要压缩以及超大响应降至 2 条的单次重试保持不变；MCP 诊断必须使用与真实检索相同的首次结果上限。
- 常见当前事实只建设六类稳定只读能力：本机时间、天气、新闻、金融事实/描述性分析、影视可用性和体育赛程/比分。模型只看到稳定 Iris operation；默认复用当前联网证据服务，精确天气/行情等可使用一次审核后自动冻结的 `web.domain.read` MCP 映射。该 capability 受联网开关约束且不等同于通用 `external.read`，不得扩展为自动交易、个性化投资建议或万能数据平台。
- 当前事实的补充输入遵循统一硬前置条件合同：能限定地区并披露范围的研究型请求直接执行；只有缺字段就无法确定目标的精确本地可用性请求暂停。宽泛的近期电影、新片与推荐不要求城市，也不冻结本地排片结构化工具；附近影院、排片、场次、票价和购票等意图才要求城市。天气等真正依赖地点的领域继续询问，禁止从 IP、Vault 或模型推断。
...
阶段 4 的产品边界是“管理中心审查、用户显式信任并绑定，Composer 逐 Run 授权”：MCP 的 `readOnlyHint` 只是候选前提，不是第三方实现确实无副作用的证明；工具还必须通过本地名称/Schema 审查，并由用户确认精确 provider/tool/schema 后进入白名单，Accept 再按冻结 transport/config 执行。它不是启用 provider 后自动授权；Iris 拒绝声明或 Schema 暴露写入、发送、删除、日历变更、进程或 secret 的工具，但无法独立验证已信任第三方服务端是否忠实实现声明。联网开关只控制受管的 `web.search`、`web.fetch` 与 `web.domain.read`，不隐式授予通用 `external.read`。

六类当前事实使用的 `web.domain.read` 是上述通用 MCP 边界之外的受限子集：用户必须先在管理中心把精确只读工具审核并映射到 Iris 稳定 operation，运行时才可在联网开启的 Run 中自动冻结；它不能出现在 Composer 的通用外部工具授权列表，也不能获得写入、发送、进程、日历或 secret 能力。
...
- 严格事实路径先检索并打包本轮证据；首批证据满足所需实体、时间、地域和来源字段时可以直接终局，证据不足的推荐、新闻、比较和范围检索保留有界搜索/抓取能力。Web 证据包使用独立容量预算，并保证模型可见内容、Run 关联与引用来自同一份证据。
- Harness 以既有 Run Envelope、证据账本和受限工具表面为唯一编排边界：本机日期时间走可信 runtime；当前外部事实冻结绝对时间窗与地域要求；后续搜索必须对应明确证据缺口并受次数、抓取、证据和超时预算共同约束。显式授权材料可用于消解联网查询，自动本地召回内容不得外发。成功的等价工具调用不重复执行，当前工具证据不得被叙述为可见历史对话。
