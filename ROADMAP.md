...
- WebEvidenceBroker 统一联网语义层：显式映射 MCP Provider 优先，DuckDuckGo 作为唯一原生托底；MiniMax 退回普通 LLM provider；助手面只保留 `web_search`，搜索、抓取、URL 深读和证据输出都经 broker 进入现有证据包
