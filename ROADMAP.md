...
- 首轮原生支持 RSS、Atom、JSON Feed，完成安全发现、条件请求、增量同步、失败退避、去重更新、FTS 搜索与 OPML 往返。摘要型来源默认启用通用网页正文补全；新文章自动补全，升级前已保存的摘要只在用户打开时补全当前一篇。用户可逐源关闭；不承诺微信历史库搜索、登录绕过、付费墙处理或站点特例。
