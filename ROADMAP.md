...
- Direct 继续保持 1 次模型、0 工具；普通 ToolLoop 使用现有 8 次模型、24 次工具总上限，并增加 local 12、network 6、external-read 6、runtime 4 的冻结分类上限。Web、本地检索、runtime 和外部只读工具共享同一个 `AgentToolLoop`；相同成功调用不重复，相同失败最多两次，连续两轮无新资源或内容后关闭工具并保留最后一次综合。
