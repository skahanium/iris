...
- Web 工具面拆为 `web_search { query }` 与 `web_fetch { urls }` 两个单一职责动作，共用现有授权、network 预算、Broker 和冻结 Provider 顺序。搜索结果先成为每次最多 4 个、每 Run 最多 8 个的临时候选，不占最终 12 条 evidence 容量；模型选择本 Run 候选或用户明确 URL 并成功抓取正文后才登记可引用来源。搜索片段、未选候选和抓取失败页面不能支持最终结论；部分失败必须把成功正文、失败 URL 与剩余要求一起返回模型。
