...
| WEB-001   | P0     | Partial    | 首次搜索计入预算，查询哈希、业务轮次和 winner 已持久化并可恢复；搜索片段已可解析时效标签供 News WebFallback，服务层已能由 headless MCP 产出验证记录；Provider failover/重试仍缺真实端到端计量夹具 | 计划 04 增加 Provider attempt/winner 生产回归                   |
