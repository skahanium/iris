...
| EVID-004  | `exact_binding_rejects_stale_or_foreign_run_evidence`                            | 证据集成       | 旧 Run/失效证据不能成为 Exact（`current_run_citation_links_exclude_foreign_and_retired_evidence` 已新增，GREEN）        |
| SEC-001   | `permission_mapping_matches_catalog_capability`                                  | 目录单元       | 每个可执行工具映射与目录声明一致（`harness_tools_do_not_inherit_vault_search_permission` GREEN）                        |
